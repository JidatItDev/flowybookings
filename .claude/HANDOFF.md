...
- New `POST /api/billing/plan-downgrade-cancel` endpoint + `resolveDowngradeCancelPreflight()` (in `plan-downgrade-decision.ts`, 3 new tests) — lets an owner undo a still-pending downgrade with zero Mollie interaction (Mollie was never touched to begin with). Wired into `ShopBillingCard.tsx` as a "Keep current plan" link next to the existing "Scheduled: X on Y" line.
- `UpgradePage.tsx`: a pricing tile matching `activeShop.pending_plan` now shows a "Scheduled" badge and disables its button, instead of allowing a repeat downgrade call.
- **Verified live 2026-08-25**, fully passing (D1, D2, P1 all confirmed on the real test shop `e643c118-e74c-41fc-a6b3-32fc6707a881`): scheduling a downgrade now touches zero Mollie state (confirmed via dashboard — no charge, no subscription change); "Keep current plan" correctly clears `pending_plan`/`pending_plan_effective_at` with no Mollie call; backdating `pending_plan_effective_at` + manually hitting `/hooks/billing-expiry` correctly applied the plan and patched Mollie's subscription price with no duplicate/premature charge. See `docs/superpowers/plans/2026-08-20-billing-e2e-matrix.md`'s 2026-08-25 rows for full detail, including the (reasoned, not 100%-Mollie-doc-confirmed) explanation for why the very first patch on a never-yet-charged subscription forced an immediate charge but later patches on an already-charged one didn't.
- 240/240 tests passing, `npm run build` clean, `tsc`/`eslint` show only the same pre-existing unrelated issues.
