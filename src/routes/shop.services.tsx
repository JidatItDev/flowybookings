            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className={cn(
                    "inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-xs font-medium transition",
                    s.is_active
              ...
              <div className="mt-4 flex items-end justify-between">
                <div>
                  <p className="text-2xl font-semibold tracking-tight">
                    {formatCents(s.price_cents, s.currency)}
                  </p>
                  {s.deposit_cents > 0 && (
                    <p className="text-xs text-muted-foreground">
                      {formatCents(s.deposit_cents, s.currency)} deposit
                    </p>
                  )}
                </div>
      ...
      <AlertDialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              "{deleting?.name}" will be removed. Existing bookings keep their pricing.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function ServiceFormDialog({
  ...
  const reset = (s: ServiceRow | null) =>
    setForm({
      name: s?.name ?? "",
      category: s?.category ?? "",
      duration_minutes: s?.duration_minutes ?? 30,
      price: s ? s.price_cents / 100 : 0,
      deposit: s ? s.deposit_cents / 100 : 0,
      is_active: s?.is_active ?? true,
      description: s?.description ?? "",
    });

  // Trigger reset when dialog opens with new data
  const [lastId, setLastId] = useState<string | null>(null);
  if (open && service?.id !== lastId) {
    setLastId(service?.id ?? null);
    reset(service);
  }
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Set the name, duration and pricing for this service.
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-2">
          ...
          <div className="flex items-center justify-between rounded-xl border border-border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Bookable by customers</p>
            </div>
            <Switch
