  ...
  const { data: shops = [], isLoading: shopsQueryLoading } = useQuery({
    ...
    queryFn: async (): Promise<ShopRow[]> => {
      ...
      const ownPromise = supabase
        .from("shops")
        .select("id, name, slug, status, plan, plan_expires_at, plan_billing_cycle, next_billing_at, subscription_status, pending_plan, pending_plan_effective_at, onboarding, policy_accepted_at, policy_version, owner_id, is_demo, created_at")
        .eq("owner_id", userId!);
