export const en: Record<string, string> = {
  // ── Global ──
  "app.name": "FlowyBookings",
  "app.tagline": "The booking platform that grows with your shop",
  "app.copyright": "© {year}",
  "nav.features": "Features",
  "nav.pricing": "Pricing",
  "nav.lovedBy": "Why us",
  "nav.signIn": "Sign in",
  "nav.getStarted": "Get started",
  "nav.goToDashboard": "Go to your dashboard",
  "nav.terms": "Terms",
  "nav.privacy": "Privacy",
  "nav.status": "Status",

  // ── Landing hero ──
  "landing.badge": "New: WhatsApp reminders → fewer no-shows",
  "landing.headline": "More bookings. Fewer no-shows.",
  "landing.headlineAccent": "All in one system.",
  "landing.sub":
    "FlowyBookings helps salons and service businesses manage their calendar, customers and payments — without the hassle.",
  "landing.proof.title": "Trusted by growing salons",
  "landing.proof.sub": "Real results that businesses across the Benelux feel from day one.",
  "landing.proof.stat1.value": "+32%",
  "landing.proof.stat1.label": "More bookings",
  "landing.proof.stat1.desc": "24/7 online booking fills the slow hours automatically.",
  "landing.proof.stat2.value": "−71%",
  "landing.proof.stat2.label": "Fewer no-shows",
  "landing.proof.stat2.desc": "Automatic WhatsApp & email reminders — proven to work.",
  "landing.proof.stat3.value": "100%",
  "landing.proof.stat3.label": "More clarity",
  "landing.proof.stat3.desc": "One calendar for your whole team. No more sticky notes.",
  "landing.ps.title": "Sound familiar?",
  "landing.ps.sub": "We built FlowyBookings to fix the daily frustrations of running a busy shop.",
  "landing.ps.problemsTitle": "Without FlowyBookings",
  "landing.ps.solutionsTitle": "With FlowyBookings",
  "landing.ps.p1": "No-shows cost you money",
  "landing.ps.p2": "Calendar chaos",
  "landing.ps.p3": "Customers forget appointments",
  "landing.ps.p4": "Messy payment tracking",
  "landing.ps.s1": "Automatic WhatsApp reminders",
  "landing.ps.s2": "Smart, clear calendar",
  "landing.ps.s3": "Online bookings, 24/7",
  "landing.ps.s4": "Payments handled via Mollie",
  "landing.appFeel.title": "Feels like an app. Works everywhere.",
  "landing.appFeel.sub":
    "Use FlowyBookings like a real app — fast flows, swipe interactions and a smooth mobile experience.",
  "landing.appFeel.b1": "Fast flows, no loading screens",
  "landing.appFeel.b2": "Swipe-friendly on mobile",
  "landing.appFeel.b3": "Works on phone, tablet and desktop",
  "landing.transparency.title": "No surprises. Ever.",
  "landing.transparency.sub":
    "You only pay for successful bookings. No hidden fees, no fine print.",
  "landing.transparency.b1": "Flat monthly price",
  "landing.transparency.b2": "Cancel anytime",
  "landing.transparency.b3": "No charge for no-shows",
  "landing.faq.title": "Frequently asked questions",
  "landing.faq.q1": "Can I switch plans later?",
  "landing.faq.a1":
    "Yes. Upgrade or downgrade anytime from your dashboard — the change is instant.",
  "landing.faq.q2": "What happens to my data?",
  "landing.faq.a2":
    "Your data is yours — hosted in the EU. We never sell customer data and you can export everything at any time.",
  "landing.faq.q3": "Are there setup fees?",
  "landing.faq.a3":
    "No. No setup fees, no contracts. Start with a 14-day free trial and then pay monthly.",
  "landing.faq.q4": "Does it work on mobile?",
  "landing.faq.a4":
    "Yes. FlowyBookings is fully mobile-first. Run your shop from your phone, or let customers book through your own page.",
  "landing.finalCta.title": "Ready to get more bookings?",
  "landing.finalCta.sub": "Start today for free — your next booking is closer than you think.",
  "landing.tryBooking": "Try the booking flow",
  "landing.seeDashboard": "See the dashboard",
  "landing.trial": "14-day free trial",
  "landing.noCreditCard": "No credit card",
  "landing.cancelAnytime": "Cancel anytime",
  "landing.viewLiveDemo": "View live demo",
  "landing.startFreeTrial": "Start free trial",
  "landing.demoHint": "No account needed — book in under 60 seconds.",
  "demo.badge": "Live demo",
  "demo.pickTitle": "Pick a demo business",
  "demo.pickSub": "Two example businesses — pick one to experience the booking flow.",
  "demo.openShop": "Open shop",
  "demo.notReady": "No demo shop available",
  "demo.notReadySub": "Open the standard booking flow to try FlowyBookings.",
  "demo.openBooking": "Open booking flow",
  "demo.bannerTitle": "You're in the live demo",
  "demo.bannerSub": "Real booking, no real payment.",
  "demo.startTrial": "Start your trial",
  "demo.paymentNotice":
    "Demo payment — no real transaction. The booking is saved so you can see how it looks in your dashboard.",
  "demo.convertTitle": "Want this for your own business?",
  "demo.convertSub": "Start 14 days free with FlowyBookings — no credit card required.",
  "demo.startTrialCta": "Start free trial",
  "demo.viewDashboard": "See in dashboard",
  "demo.safeNotice": "Bookings in this demo are stored, but no real payment is processed.",
  "landing.today": "Today",
  "landing.fromBookings": "From {count} confirmed bookings",
  "landing.bookingsWeek": "Bookings this week",
  "landing.revenueWeek": "Revenue this week",
  "landing.activeCustomers": "Active customers",
  "landing.remindersSent": "Reminders sent",

  // ── Features ──
  "features.title": "Everything your shop needs, beautifully organized",
  "features.sub":
    "From the first tap to the final tip — FlowyBookings handles every moment of the customer journey.",
  "features.smartBookings": "Smart bookings",
  "features.smartBookingsDesc": "Daily and weekly calendar, drag-and-drop, conflict detection.",
  "features.customerProfiles": "Customer profiles",
  "features.customerProfilesDesc": "Notes, preferences, history and lifetime spend in one view.",
  "features.payments": "Payments & deposits",
  "features.paymentsDesc":
    "Mollie integration with iDEAL, Bancontact and credit card. Track paid, unpaid and refunds.",
  "features.reminders": "Reminders that work",
  "features.remindersDesc": "Email, SMS and WhatsApp at 24h and 2h before the slot.",
  "features.analytics": "Insightful analytics",
  "features.analyticsDesc": "Revenue, no-shows, busy days, top services and returning customers.",
  "features.mobile": "Mobile-first",
  "features.mobileDesc": "Polished on every screen — your shop, in your pocket.",

  // ── Built for ──
  "builtFor.title": "Built for service businesses of every shape",
  "builtFor.tattoo": "Tattoo",
  "builtFor.barber": "Barber",
  "builtFor.nails": "Nails",
  "builtFor.beauty": "Beauty",
  "builtFor.hair": "Hair",
  "builtFor.petGrooming": "Pet grooming",

  // ── Why us (real value props, no fake testimonials) ──
  "whyUs.title": "Built for the Benelux, not Silicon Valley",
  "whyUs.sub": "No made-up reviews. Just concrete benefits you feel from day one.",
  "whyUs.localTitle": "Local payments",
  "whyUs.localDesc": "iDEAL, Bancontact and credit card via Mollie — no clunky US-only providers.",
  "whyUs.noShowTitle": "Fewer no-shows",
  "whyUs.noShowDesc":
    "Automatic reminders 24h and 2h before the slot. Optional deposit per customer.",
  "whyUs.fastTitle": "Book in 60 seconds",
  "whyUs.fastDesc":
    "Mobile-first flow with no account required. Customers pick a service, a time, and they're done.",
  "whyUs.ownTitle": "Your data, your customers",
  "whyUs.ownDesc": "Fully EU-hosted. We never sell customer data and use no tracking cookies.",
  "whyUs.transparentTitle": "Transparent pricing",
  "whyUs.transparentDesc":
    "Flat monthly fee plus a low transaction fee. No hidden costs or multi-year contracts.",
  "whyUs.supportTitle": "Dutch-speaking support",
  "whyUs.supportDesc": "Real humans who understand your business — reachable by email and chat.",

  // ── Pricing ──
  "pricing.title": "Fair pricing, fixed fee per booking",
  "pricing.sub":
    "No percentage of your revenue. One predictable booking fee — regardless of the amount.",
  "pricing.mostPopular": "Most chosen",
  "pricing.transactionFee": "booking fee per booking",
  "pricing.noFee": "No booking fee",
  "pricing.period.trial": "14 days",
  "pricing.period.month": "month",
  "pricing.cta.trial": "Start free trial",
  "pricing.cta.starter": "Choose Starter",
  "pricing.cta.pro": "Choose Pro",
  "pricing.cta.premium": "Choose Premium",
  "pricing.feat.trial.staff": "1 staff member",
  "pricing.feat.trial.bookings": "Up to 30 bookings",
  "pricing.feat.trial.reminders": "Email reminders",
  "pricing.feat.trial.fee": "No booking fee during trial",
  "pricing.feat.starter.staff": "3 staff members",
  "pricing.feat.starter.bookings": "Unlimited bookings",
  "pricing.feat.starter.reminders": "SMS + email reminders",
  "pricing.feat.starter.fee": "Booking fee: €0.50 per booking",
  "pricing.feat.pro.staff": "10 staff members",
  "pricing.feat.pro.bookings": "Unlimited bookings",
  "pricing.feat.pro.reminders": "WhatsApp + SMS + email reminders",
  "pricing.feat.pro.analytics": "Advanced analytics",
  "pricing.feat.pro.branding": "Custom branding",
  "pricing.feat.pro.fee": "Booking fee: €0.30 per booking",
  "pricing.feat.premium.staff": "Unlimited staff",
  "pricing.feat.premium.multilocation": "Multi-location",
  "pricing.feat.premium.support": "Priority support",
  "pricing.feat.premium.api": "API access",
  "pricing.feat.premium.fee": "No booking fee — 0% on your revenue",
  "pricing.compare.toggle.show": "Compare all features",
  "pricing.compare.toggle.hide": "Hide comparison",
  "pricing.compare.swipeHint": "Swipe horizontally to see all plans",
  "pricing.compare.feature": "Feature",
  "pricing.compare.addonsNote":
    "Add-ons (purple badges) will soon be available as separate upgrades on top of your plan.",
  "pricing.compare.cat.basic": "Basics",
  "pricing.compare.cat.limits": "Limits",
  "pricing.compare.cat.communication": "Communication",
  "pricing.compare.cat.advanced": "Advanced",
  "pricing.compare.cat.enterprise": "Enterprise",
  "pricing.compare.cat.support": "Support",
  "pricing.compare.row.bookingPage": "Online booking page",
  "pricing.compare.row.calendar": "Calendar & appointments",
  "pricing.compare.row.customers": "Customer database",
  "pricing.compare.row.mollie": "Mollie integration",
  "pricing.compare.row.emailReminders": "Email reminders",
  "pricing.compare.row.bookingsPerMonth": "Bookings per month",
  "pricing.compare.row.staff": "Staff members",
  "pricing.compare.row.bookingFee": "Booking fee per booking",
  "pricing.compare.row.smsReminders": "SMS reminders",
  "pricing.compare.row.whatsapp": "WhatsApp reminders",
  "pricing.compare.row.marketingEmails": "Marketing emails",
  "pricing.compare.row.analytics": "Advanced analytics",
  "pricing.compare.row.branding": "Custom branding",
  "pricing.compare.row.reviews": "Google Reviews",
  "pricing.compare.row.waitlist": "Waitlist",
  "pricing.compare.row.multiLocation": "Multi-location",
  "pricing.compare.row.whiteLabel": "White-label",
  "pricing.compare.row.api": "API access",
  "pricing.compare.row.stripe": "Stripe (international)",
  "pricing.compare.row.emailSupport": "Email support",
  "pricing.compare.row.chatSupport": "Chat support",
  "pricing.compare.row.phoneSupport": "Phone support",
  "pricing.compare.row.accountManager": "Account manager",
  "pricing.compare.val.included": "Included",
  "pricing.compare.val.notIncluded": "Not included",
  "pricing.compare.val.soon": "Coming soon",
  "pricing.compare.val.unlimited": "∞",
  "pricing.compare.val.noFee": "€0",
  "pricing.compare.val.starterFee": "€0.50",
  "pricing.compare.val.proFee": "€0.30",
  "pricing.footnote":
    "Fixed booking fee per successful booking — regardless of the amount. Standard Mollie transaction fees (e.g. €0.29 for iDEAL) still apply and run through your own Mollie account. Your money always lands directly in your account.",

  // ── CTA ──
  "cta.title": "Ready to fill your calendar?",
  "cta.sub": "Try FlowyBookings free for 14 days. No credit card, no commitment.",
  "cta.tryBooking": "Try the booking flow",
  "cta.exploreDashboard": "Explore dashboard →",

  // ── Auth ──
  "auth.createAccount": "Create your account",
  "auth.trialLine": "Start your 14-day free trial. No credit card.",
  "auth.fullName": "Full name",
  "auth.email": "Email",
  "auth.password": "Password",
  "auth.showPassword": "Show password",
  "auth.hidePassword": "Hide password",
  "auth.minChars": "At least 8 characters.",
  "auth.createBtn": "Create account",
  "auth.haveAccount": "Already have an account?",
  "auth.signIn": "Sign in",
  "auth.welcomeBack": "Welcome back",
  "auth.loginFailedTitle": "Sign-in didn't go through",
  "auth.loginFailedMessage":
    "Your session expired or the sign-in couldn't be confirmed. Please try again.",
  "auth.loginRetry": "Sign in again",
  "auth.backToHome": "Back to homepage",
  "auth.signInSub": "Sign in to your dashboard.",
  "auth.signInBtn": "Sign in",
  "auth.newHere": "New here?",
  "auth.createAnAccount": "Create an account",
  "auth.demoAccounts": "Demo accounts",
  "auth.password_label": "Password",
  "auth.firstTime": "First time?",
  "auth.provisionDemo": "Provision demo users",
  "auth.seeding": "Seeding…",
  "auth.accountCreated": "Account created — you can sign in now.",
  "auth.checkEmailTitle": "Check your email",
  "auth.checkEmailBody":
    "We sent a confirmation link to {email}. Click it to activate your account.",
  "auth.checkEmailHint": "After confirming, you'll set up your shop and start your free trial.",
  "auth.checkEmailResend": "Resend confirmation email",
  "auth.checkEmailResending": "Sending…",
  "auth.checkEmailResent": "Confirmation email sent again.",
  "auth.accountExistsTitle": "Account already exists",
  "auth.accountExistsBody":
    "An account with {email} is already registered. Sign in instead, or reset your password if you forgot it.",
  "auth.forgotPasswordCta": "Forgot password?",
  "auth.agreePrefix": "I agree to the",
  "auth.termsLink": "Terms",
  "auth.and": "and",
  "auth.privacyLink": "Privacy Policy",
  "auth.mustAgree": "Please agree to the Terms & Privacy Policy to continue.",
  "auth.continueWithGoogle": "Continue with Google",
  "auth.orContinueWith": "or continue with email",
  "auth.googleNotEnabled":
    "Google login isn't enabled yet. Turn on Google in Authentication settings.",
  "auth.googleResetDone": "Google button reset. It will reappear on the login page.",
  // ── Booking link widget ──
  "bookingLink.title": "Share your booking link",
  "bookingLink.subtitle": "Customers can book an appointment online with one click.",
  "bookingLink.label": "Booking link",
  "bookingLink.live": "Live",
  "bookingLink.copy": "Copy",
  "bookingLink.copiedShort": "Copied",
  "bookingLink.copied": "Booking link copied",
  "bookingLink.copyFailed": "Copy failed — please select and copy manually.",
  "bookingLink.shareMore": "More",
  "bookingLink.shareTitle": "Book at {shop}",
  "bookingLink.shareMsg": "Book your appointment at {shop}: {url}",
  "bookingLink.igCopied": "Link copied — paste it in your Instagram bio or story.",
  "bookingLink.qr": "QR code",
  "bookingLink.qrTitle": "Print your QR code",
  "bookingLink.qrDesc": "Hang it up in your shop — customers scan and book instantly.",
  "bookingLink.qrDownload": "Download as PNG",
  "bookingLink.posterDownload": "Print poster (A5)",
  "bookingLink.posterLoading": "One moment…",
  "bookingLink.posterReady": "Poster downloaded — ready to print.",
  "bookingLink.posterError": "Could not create the poster. Please try again.",
  "bookingLink.posterHint": "A5 PDF — perfect to display in your shop.",
  "bookingLink.posterHeadline": "Book your appointment online",
  "bookingLink.posterScanHint": "Scan the QR code with your phone",
  "bookingLink.posterOrVisit": "or visit",
  "bookingLink.posterPoweredBy": "Powered by FlowyBookings",
  "bookingLink.noSlug": "Set a shop URL first to share your booking link.",
  "legal.reconsent.title": "Our policies have been updated",
  "legal.reconsent.body":
    "Please review and accept the latest versions of the policies below to continue using FlowyBookings.",
  "legal.reconsent.doc.privacy": "Privacy Policy",
  "legal.reconsent.doc.terms": "Terms of Service",
  "legal.reconsent.doc.refunds": "Refund Policy",
  "legal.reconsent.agree": "I have read and accept the updated policies.",
  "legal.reconsent.agreeAria": "Accept updated policies",
  "legal.reconsent.accept": "Accept and continue",
  "legal.reconsent.accepted": "Thanks — your acceptance has been recorded.",
  "auth.welcomeBackToast": "Welcome back",
  "auth.demoReady": "Demo users ready — pick one and click Sign in.",
  "auth.shopOwner": "Shop owner",
  "auth.staff": "Staff",
  "auth.signOut": "Sign out",
  "auth.forgotPassword": "Forgot password?",
  "auth.forgotTitle": "Forgot your password",
  "auth.forgotSub": "Enter your email and we'll send you a link to reset your password.",
  "auth.sendResetLink": "Send reset link",
  "auth.sending": "Sending…",
  "auth.resetEmailSent": "Check your email for the reset link.",
  "auth.backToSignIn": "Back to sign in",
  "auth.resetTitle": "Set a new password",
  "auth.resetSub": "Pick a strong password of at least 8 characters.",
  "auth.newPassword": "New password",
  "auth.confirmPassword": "Confirm password",
  "auth.updatePassword": "Update password",
  "auth.updating": "Updating…",
  "auth.passwordsDontMatch": "Passwords don't match.",
  "auth.passwordTooShort": "Password must be at least 8 characters.",
  "auth.passwordUpdated": "Password updated — you're signed in.",
  "auth.invalidResetLink": "This reset link is invalid or expired. Request a new one.",
  "auth.signedInAs": "Signed in as",
  "auth.invalidCredentials": "Incorrect email or password.",
  "auth.emailExistsTitle": "An account with this email already exists",
  "auth.emailExistsBody": "Would you like to sign in with this account?",
  "auth.goToLogin": "Go to sign in",
  "auth.trialBadge": "14 days free",
  "auth.startTrialTitle": "Start your free trial",
  "auth.benefitNoCard": "No credit card required",
  "auth.benefitFullAccess": "Full access to all features",
  "auth.benefitCancelAnytime": "Cancel anytime",

  // ── Admin login ──
  "admin.platformAccess": "Platform access",
  "admin.authorizedOnly": "Authorized personnel only.",
  "admin.signIn": "Sign in",

  // ── Shop sidebar / nav ──
  "shopNav.dashboard": "Dashboard",
  "shopNav.calendar": "Calendar",
  "shopNav.customers": "Customers",
  "shopNav.services": "Services",
  "shopNav.staff": "Staff",
  "shopNav.payments": "Payments",
  "shopNav.analytics": "Analytics",
  "shopNav.notifications": "Notifications",
  "shopNav.billing": "Billing",
  "shopNav.settings": "Settings",
  "shopNav.support": "Support",
  "shopNav.searchPlaceholder": "Search bookings, customers…",
  "shopNav.switchToAdmin": "Switch to Admin →",
  "shopNav.upgradePremium": "Upgrade to Premium",
  "shopNav.upgradeSub": "Unlock multi-location, API access and priority support.",
  "shopNav.seePlans": "See plans",
  "shopNav.subscription": "Subscription",
  "shopNav.lockedTooltip": "Resubscribe to unlock",
  "account.menuLabel": "Open account menu",

  // ── Admin sidebar / nav ──
  "adminNav.overview": "Overview",
  "adminNav.shops": "Shops",
  "adminNav.users": "Users",
  "adminNav.bookings": "Bookings",
  "adminNav.payments": "Payments",
  "adminNav.plans": "Plans",
  "adminNav.messages": "Messages",
  "adminNav.support": "Support",
  "adminNav.activity": "Activity",
  "adminNav.auditLogs": "Audit log",
  "adminNav.demo": "Demo controls",
  "adminNav.security": "Security & Access",
  "adminNav.settings": "Settings",
  "activityPage.title": "Activity",
  "activityPage.subtitle": "Last {limit} platform events with filters and CSV export.",
  "activityPage.exportCsv": "Export CSV",
  "activityPage.filterAction": "Action:",
  "activityPage.filterShop": "Shop:",
  "activityPage.allActions": "All actions",
  "activityPage.allShops": "All shops",
  "activityPage.resultsCount": "{shown} of {total} events",
  "activityPage.tableTitle": "Events",
  "activityPage.empty": "No events match these filters.",
  "activityPage.col.time": "Time (UTC)",
  "activityPage.col.action": "Action",
  "activityPage.col.shop": "Shop",
  "activityPage.col.actor": "User",
  "activityPage.col.metadata": "Details",
  "activityPage.pageOf": "Page {page} of {total}",
  "activityPage.showingRange": "Showing {from}–{to} of {total}",
  "activityPage.prev": "Previous",
  "activityPage.next": "Next",
  "activityPage.filterDateFrom": "From:",
  "activityPage.filterDateTo": "To:",
  "activityPage.pickDate": "Pick a date",
  "activityPage.clearDates": "Clear dates",
  "adminNav.superAdmin": "SUPER ADMIN",
  "adminNav.platform": "FlowyBookings platform",
  "adminNav.platformHealthy": "Platform healthy",
  "adminNav.allSystems": "All systems operational · 99.98% uptime",
  "adminNav.switchToShop": "Switch to Shop →",
  "adminNav.platformOwner": "Platform owner",
  "adminNav.alertSound": "Alert sound",

  // ── Shop Dashboard ──
  "dashboard.welcomeBack": "Welcome back, {name} 👋",
  "dashboard.title": "Dashboard",
  "dashboard.subWithShop": "Here's what's happening today at {name}.",
  "dashboard.subNoShop": "Pick a shop to see today's activity.",
  "dashboard.newBooking": "New booking",
  "dashboard.todayBookings": "Today's bookings",
  "dashboard.todayRevenue": "Today's revenue",
  "dashboard.pending": "Pending",
  "dashboard.needsReview": "Needs review",
  "dashboard.noShows7d": "No-shows (7d)",
  "dashboard.weeklyRevenue": "Weekly revenue",
  "dashboard.last7days": "Last 7 days (sample)",
  "dashboard.livePreview": "Live preview",
  "dashboard.live": "Live",
  "dashboard.atAGlance": "At a glance",
  "dashboard.customers": "Customers",
  "dashboard.activeServices": "Active services",
  "dashboard.activeStaff": "Active staff",
  "dashboard.totalBookings": "Total bookings",
  "dashboard.todayAppointments": "Today's appointments",
  "dashboard.viewAll": "View all",
  "dashboard.noAppointments": "No appointments today.",
  "dashboard.unknown": "Unknown",
  "dashboard.totalCustomers": "Total customers",
  "dashboard.noShowRate30d": "No-show rate (30d)",
  "dashboard.monthRevenue": "Revenue this month",
  "dashboard.weekRevenue": "Revenue this week",
  "dashboard.upcoming": "Upcoming bookings",
  "dashboard.upcomingHint": "Next 7 days",
  "dashboard.cancellations30d": "Cancellations (30d)",
  "dashboard.bookingsCount": "{n} bookings",
  "dashboard.staffPerformance": "Staff performance",
  "dashboard.staffPerformanceHint": "Bookings & revenue per staff (last 30 days)",
  "dashboard.occupancy": "Occupancy",
  "dashboard.occupancyHint": "Booked vs. available time, last 7 days",
  "dashboard.occupancyBooked": "Booked: {hours}",
  "dashboard.occupancyAvailable": "Available: {hours}",
  "dashboard.occupancyNoHours": "Set staff working hours to see occupancy.",
  "calendar.occupancyDay": "Occupancy {pct}% — {booked} of {available}",
  "calendar.occupancyStaff": "{name}: {pct}% booked",
  "calendar.occupancyNoData": "No working hours set",

  // ── Onboarding checklist ──
  "checklist.badge": "Get started",
  "checklist.title": "Finish setting up your shop",
  "checklist.progress": "{done} of {total} steps complete",
  "checklist.dismiss": "Dismiss",
  "checklist.serviceTitle": "Add your first service",
  "checklist.serviceDesc": "What do customers book? Add price and duration.",
  "checklist.serviceCta": "Add service",
  "checklist.staffTitle": "Add a team member",
  "checklist.staffDesc": "Even if it's just you — staff power the calendar.",
  "checklist.staffCta": "Add staff",
  "checklist.shareTitle": "Share your booking link",
  "checklist.shareDesc": "Send it to customers to start taking bookings today.",
  "checklist.shareCta": "Copy link",
  "checklist.linkCopied": "Booking link copied to clipboard",
  "checklist.copyFailed": "Couldn't copy the link",
  "checklist.hoursTitle": "Set your opening hours",
  "checklist.hoursDesc": "Customers can only book within your working hours.",
  "checklist.hoursCta": "Opening hours",
  "checklist.paymentsTitle": "Connect payments",
  "checklist.paymentsDesc": "Optional — collect deposits to reduce no-shows.",
  "checklist.paymentsCta": "Connect",
  "checklist.optional": "Optional",
  "checklist.optionalLabel": "Want more? Activate these extras",
  "checklist.skip": "Skip",
  "checklist.successBadge": "Ready for bookings",
  "checklist.successTitle": "You're ready to take bookings 🎉",
  "checklist.successDesc":
    "Your shop is fully set up. Create your first appointment and start earning.",
  "checklist.successCta": "Create first booking",

  // ── Dashboard insights ──
  "insights.lostNoShowsTitle": "Lost from no-shows this month",
  "insights.lostNoShowsHint": "Estimated revenue missed from no-show bookings.",
  "insights.popularServiceTitle": "Most popular service",
  "insights.popularServiceHint": "{n} bookings to date.",
  "insights.busiestDayTitle": "Busiest day",
  "insights.busiestDayHint": "Based on the last 90 days.",
  "insights.notEnoughData": "Not enough data yet.",
  "insights.revenueTitle": "Revenue",
  "insights.totalRevenue": "Total revenue",
  "insights.monthRevenue": "This month",
  "insights.revenuePerService": "Top services by revenue",
  "insights.bookingsTitle": "Bookings",
  "insights.totalBookingsHint": "All-time bookings",
  "insights.completed": "Completed",
  "insights.cancelled": "Cancelled",
  "insights.noShow": "No-show",
  "insights.confirmed": "Confirmed",
  "insights.pending": "Pending",
  "insights.customersTitle": "Customers",
  "insights.returning": "Returning",
  "insights.newCustomers": "New",
  "insights.returningPct": "{pct}% of customers come back.",
  "insights.topCustomers": "Top customers by spend",
  "weekday.sun": "Sunday",
  "weekday.mon": "Monday",
  "weekday.tue": "Tuesday",
  "weekday.wed": "Wednesday",
  "weekday.thu": "Thursday",
  "weekday.fri": "Friday",
  "weekday.sat": "Saturday",

  // ── Calendar ──
  "calendar.title": "Calendar",
  "calendar.description": "View and manage upcoming appointments.",
  "calendar.newBooking": "New booking",
  "calendar.live": "Live",
  "calendar.liveTooltip": "Changes appear instantly",
  "calendar.liveConnecting": "Connecting…",
  "calendar.liveConnectingTooltip": "Establishing live connection",
  "calendar.liveOffline": "Offline",
  "calendar.liveOfflineTooltip": "Live updates paused — refresh if needed",
  "calendar.filterAll": "all",
  "calendar.pending": "pending",
  "calendar.confirmed": "confirmed",
  "calendar.completed": "completed",
  "calendar.cancelled": "cancelled",
  "calendar.noShow": "no-show",
  "calendar.visualStatus.paymentPending": "Awaiting payment",
  "calendar.visualStatus.confirmationPending": "Needs confirmation",
  "calendar.needsAttention": "needs your attention",
  "calendar.needsAttentionNone": "Nothing needs attention",
  "calendar.legendTitle": "Status legend",
  "calendar.allUpcoming": "All upcoming",
  "calendar.noBookings": "No bookings yet",
  "calendar.noBookingsDesc": "Create your first booking to get started.",
  "calendar.noMatch": "No bookings match this filter",
  "calendar.noMatchDesc": "Try a different status filter.",
  "calendar.loadErrorTitle": "Couldn't load bookings",
  "calendar.loadErrorDesc": "Check your internet connection and try again.",
  "calendar.retry": "Try again",
  "calendar.retrying": "Loading…",
  "calendar.when": "When",
  "calendar.customer": "Customer",
  "calendar.service": "Service",
  "calendar.staffCol": "Staff",
  "calendar.status": "Status",
  "calendar.editBooking": "Edit booking",
  "calendar.newBookingTitle": "New booking",
  "calendar.cancel": "Cancel",
  "calendar.bookingUpdated": "Booking updated",
  "calendar.resizeHandle": "Drag to adjust duration",
  "calendar.dropInvalidClosed": "Not working this day",
  "calendar.dropInvalidOffHours": "Outside working hours ({range})",
  "calendar.dropInvalidBreak": "Break {range}",
  "calendar.dropInvalidConflict": "Conflict with {range}",
  "calendar.dropBlocked": "Move blocked: {reason}",
  "calendar.bookingCreated": "Booking created",
  "calendar.conflictWith":
    "Conflict: {staff} already has a booking with {customer} ({service}) from {range}.",
  "calendar.conflictGeneric":
    "This staff member already has an overlapping booking in this time slot. Pick another time or staff member.",
  "bookingError.conflict": "Time slot already taken. Pick another time or staff member.",
  "bookingError.outsideHours": "Outside this staff member's working hours.",
  "bookingError.outsideHoursRange":
    "Outside working hours ({range}). Pick a time within the schedule.",
  "bookingError.duringBreak": "Conflicts with this staff member's break.",
  "bookingError.duringBreakRange": "Conflicts with break ({range}). Pick another time.",
  "bookingError.closedDay":
    "Staff is not available on the selected day. Pick another day or staff member.",
  "calendar.pickCustomer": "Pick customer",
  "calendar.pickService": "Pick service",
  "calendar.pickStaff": "Pick staff",
  "calendar.searchService": "Search service…",
  "calendar.searchStaff": "Search staff…",
  "calendar.noServiceMatch": "No service found",
  "calendar.noStaffMatch": "No staff found",
  "calendar.start": "Start",
  "calendar.duration": "Duration (min)",
  "calendar.notes": "Notes",
  "calendar.save": "Save",
  "calendar.saving": "Saving…",
  "calendar.edit": "Edit",
  "calendar.view": "View",
  "calendar.reschedule": "Reschedule",
  "calendar.markNoShow": "Mark as no-show",

  // ── Customers ──
  "customers.title": "Customers",
  "customers.description": "Profiles, history and lifetime value.",
  "customers.newCustomer": "New customer",
  "customers.searchPlaceholder": "Search customers…",
  "customers.noMatches": "No matches",
  "customers.noCustomers": "No customers yet",
  "customers.noCustomersDesc": "Add your first customer or let them book online.",
  "customers.noMatchDesc": "Try a different search term.",
  "customers.addCustomer": "Add customer",
  "customers.customerCol": "Customer",
  "customers.contact": "Contact",
  "customers.totalSpent": "Total spent",
  "customers.lastVisit": "Last visit",
  "customers.editCustomer": "Edit customer",
  "customers.newCustomerTitle": "New customer",
  "customers.deleteTitle": "Delete {name}?",
  "customers.deleteDesc":
    "Their booking history will be kept but no longer linked to a customer profile.",
  "customers.deleteBlocked":
    "This customer has upcoming bookings, so the profile cannot be deleted. Cancel or complete those bookings first.",
  "customers.duplicateEmail":
    "A customer with this email already exists ({name}). Open that profile instead of creating a duplicate.",
  "customers.duplicateEmailGeneric":
    "A customer with this email already exists in this shop.",
  "customers.cancel": "Cancel",
  "customers.delete": "Delete",
  "customers.deleted": "Customer deleted",
  "customers.updated": "Customer updated",
  "customers.added": "Customer added",
  "customers.fullName": "Full name",
  "customers.email": "Email",
  "customers.phone": "Phone",
  "customers.notes": "Notes",
  "customers.saveChanges": "Save changes",
  "customers.saving": "Saving…",
  "customers.noShows": "No-shows",
  "customers.noShowsLabel": "No-show count",
  "customers.repeatNoShow": "Repeat no-show",
  "customers.depositRequired": "Deposit required",
  "customers.requireDeposit": "Always require a deposit for this customer",
  "customers.requireDepositHint": "Recommended for customers with 2 or more no-shows.",
  "customers.filterLabel": "Filter",
  "customers.filterAll": "All customers",
  "customers.filterNew": "New customers",
  "customers.filterTop": "Top customers",
  "customers.filterRisk": "No-show risk",
  "customers.filterRecent": "Recent visit",
  "customers.badgeNew": "New",
  "customers.badgeVip": "VIP",
  "customers.badgeNoShow": "No-show",
  "customers.visitsOne": "visit",
  "customers.visitsMany": "visits",
  "customers.resultsCount": "{count} customers",

  // ── Shared error messages ──
  "errors.notSignedIn": "You need to be signed in to do that.",
  "errors.noActiveShop": "No active shop selected.",
  "errors.pickStartTime": "Please pick a start time.",
  "errors.missingStaffId": "Missing staff member.",
  "errors.authFailed": "Authentication failed.",
  "errors.seedFailed": "Could not seed demo data.",

  // ── Accessibility labels ──
  "a11y.openMenu": "Open menu",
  "a11y.closeMenu": "Close menu",

  // ── Onboarding extras ──
  "onboarding.slugPlaceholder": "your-shop-name",

  "customers.tags": "Tags",
  "customers.noTags": "No tags yet — add one below.",
  "customers.addTag": "Add",
  "customers.addTagPlaceholder": "Add a tag (e.g. VIP)…",
  "customers.bookingHistory": "Booking history",
  "customers.bookingHistoryCount": "{count} bookings on record",
  "customers.noBookings": "No bookings for this customer yet.",
  "customers.serviceRemoved": "Service removed",
  "customers.profileSubtitle": "Full CRM profile, history and preferences.",
  "customers.profileNotFound": "We couldn't find this customer.",
  "customers.backToList": "Back to customers",
  "customers.customerSince": "Customer since {date}",
  "customers.bookingsLabel": "Bookings",
  "customers.completedOfTotal": "Completed of total",
  "customers.notesHint": "Private notes only visible to your team.",
  "customers.notesPlaceholder": "Allergies, preferences, favourite drinks…",
  "customers.saveNotes": "Save notes",
  "customers.preferences": "Preferences",
  "customers.preferencesHint":
    "Help your team serve this customer better — visible on every booking.",
  "customers.favoriteStaff": "Favourite staff member",
  "customers.favoriteStaffNone": "No preference",
  "customers.favoriteService": "Favourite service",
  "customers.favoriteServiceNone": "No preference",
  "customers.allergies": "Allergies / things to watch",
  "customers.allergiesPlaceholder": "E.g. PPD allergy, sensitive scalp, latex-free…",
  "customers.allergyBadge": "Allergy",
  "customers.communication": "Communication preference",
  "customers.commEmail": "Email",
  "customers.commSms": "SMS",
  "customers.commAny": "Either is fine",
  "customers.commNone": "No marketing",
  "customers.savePreferences": "Save preferences",
  "customers.preferencesSaved": "Preferences updated",
  "customers.sourceManual": "Added manually",
  "customers.sourceBookingPage": "Via booking page",
  "customers.sourceImported": "Imported from {platform}",
  "customers.avgPerVisit": "Average per visit",
  "customers.firstVisit": "First visit",
  "customers.timeline": "Timeline",
  "customers.timelineSubtitle": "All bookings and payments, newest first.",
  "customers.timelineEmpty": "No activity for this customer yet.",
  "customers.timelineCount": "{count} events",
  "customers.timelineFilterAll": "All",
  "customers.timelineFilterBookings": "Bookings",
  "customers.timelineFilterPayments": "Payments",
  "customers.timelinePaymentTitle": "Payment via {provider}",
  "customers.timelinePaymentNoProvider": "Payment",
  "customers.timelinePaymentForBooking": "For booking on {date}",
  "customers.timelinePaymentNoBooking": "Standalone payment",
  "customers.timelineDeposit": "Deposit",
  "customers.timelineRefund": "Refund",
  "customers.paymentStatus.unpaid": "Unpaid",
  "customers.paymentStatus.deposit_paid": "Deposit paid",
  "customers.paymentStatus.paid": "Paid",
  "customers.paymentStatus.refunded": "Refunded",
  "customers.paymentStatus.failed": "Failed",
  "customerImport.cta": "Import customers",
  "customerImport.title": "Import customers",
  "customerImport.tabCsv": "CSV / Excel",
  "customerImport.tabPlatform": "From another platform",
  "customerImport.tabManual": "Manually",
  "customerImport.csvIntro":
    "Upload a CSV or Excel file with your customers. We auto-detect the columns.",
  "customerImport.platformIntro":
    "Pick where you're coming from for specific instructions and column presets.",
  "customerImport.dropZone": "Drag a file here or click to choose",
  "customerImport.dropZoneShort": "Click to upload or drag here",
  "customerImport.acceptedFormats": ".csv, .xlsx or .xls — max 10 MB",
  "customerImport.fromPlatform": "Which platform are you coming from?",
  "customerImport.manualTitle": "Just a few customers?",
  "customerImport.manualHint": "Add them one by one via 'New customer'.",
  "customerImport.manualCta": "Close and use 'New customer'",
  "customerImport.fileLabel": "File",
  "customerImport.changeFile": "Choose another file",
  "customerImport.mapName": "Column for name",
  "customerImport.mapEmail": "Column for email",
  "customerImport.mapPhone": "Column for phone",
  "customerImport.mapNotes": "Column for notes",
  "customerImport.dedupTitle": "What to do with duplicates (same email or phone)?",
  "customerImport.dedupSkip": "Skip",
  "customerImport.dedupMerge": "Update (merge)",
  "customerImport.previewTitle": "Preview",
  "customerImport.firstFive": "first 5 rows",
  "customerImport.colName": "Name",
  "customerImport.colEmail": "Email",
  "customerImport.colPhone": "Phone",
  "customerImport.colNotes": "Notes",
  "customerImport.noPreviewRows": "No valid rows found — check the column mapping.",
  "customerImport.foundCount": "{count} customers found",
  "customerImport.invalidCount": "{count} rows skipped (no name)",
  "customerImport.importing": "Importing…",
  "customerImport.importNow": "Import {count} customers",
  "customerImport.resultTitle": "Import complete",
  "customerImport.resultInserted": "customers imported",
  "customerImport.resultUpdated": "existing customers updated",
  "customerImport.resultSkipped": "duplicates skipped",
  "customerImport.resultNoEmail": "without email (still added)",
  "customerImport.resultErrors": "errors",
  "customerImport.downloadErrors": "Download error report",
  "customerImport.viewImported": "View imported customers",
  "customerImport.errEmpty": "The file contains no rows.",
  "customerImport.errParse": "Could not read the file.",
  "customerImport.errNoValid": "No valid customers to import.",

  // ── Services ──
  "services.title": "Services",
  "services.description": "Manage your menu, pricing and deposits.",
  "services.addService": "Add service",
  "services.noServices": "No services yet",
  "services.noServicesDesc": "Add your first service to start accepting bookings.",
  "services.active": "Active",
  "services.inactive": "Inactive",
  "services.min": "{n} min",
  "services.deposit": "{amount} deposit",
  "services.editService": "Edit service",
  "services.newService": "New service",
  "services.deleteService": "Delete service?",
  "services.deleteDesc": '"{name}" will be removed. Existing bookings keep their pricing.',
  "services.deleteBlocked":
    '"{name}" has upcoming bookings, so it cannot be deleted. Deactivate it to hide it from new bookings.',
  "services.deleteBlockedInactive":
    '"{name}" has upcoming bookings, so it cannot be deleted. Existing bookings keep their pricing.',
  "services.deactivateInstead": "Deactivate instead",
  "services.cancel": "Cancel",
  "services.delete": "Delete",
  "services.deleted": "Service deleted",
  "services.updated": "Service updated",
  "services.created": "Service created",
  "services.name": "Name",
  "services.category": "Category",
  "services.durationMin": "Duration (min)",
  "services.priceEur": "Price (€)",
  "services.depositEur": "Deposit (€)",
  "services.descriptionLabel": "Description",
  "services.activeLabel": "Active",
  "services.bookable": "Bookable by customers",
  "services.saveChanges": "Save changes",
  "services.createService": "Create service",
  "services.saving": "Saving…",
  "services.setDetails": "Set the name, duration and pricing for this service.",
  "services.depositHint": "Optional. Customer pays this amount online when booking.",
  "services.depositTooHigh": "Deposit cannot exceed the price.",
  "services.depositNegative": "Deposit cannot be negative.",
  "services.depositModeDefault": "Shop default %",
  "services.depositModeCustom": "Custom amount",
  "services.depositModeDefaultNote": "Uses the shop's default deposit ({percent}%) at booking time.",
  "services.priceNegative": "Price cannot be negative.",
  "services.searchPlaceholder": "Search service…",
  "services.filterAll": "All",
  "services.filterPopular": "Popular",
  "services.filterPriceHigh": "Highest price",
  "services.filterDurationShort": "Shortest duration",
  "services.filterDurationLong": "Longest duration",
  "services.filterCategory": "Category",
  "services.filterAllCategories": "All categories",
  "services.bookingsCount": "{n} bookings",
  "services.noBookings": "No bookings yet",
  "services.popular": "Popular",
  "services.assignedStaff": "{n} team",
  "services.noStaff": "No team",
  "services.duplicate": "Duplicate",
  "services.duplicated": "Service duplicated",
  "services.copyOf": "Copy of {name}",
  "services.noResults": "No services found",
  "services.noResultsDesc": "Try adjusting your search or filters.",
  "services.staff": "Team",
  "services.staffHint": "Who can perform this service?",
  "services.staffNone": "No one assigned yet",
  "services.staffSelected": "{n} selected",
  "services.staffAll": "Everyone",
  "services.staffPickTitle": "Assign team",
  "services.staffPickDesc": "Select who can perform this service.",
  "services.done": "Done",
  "services.viewService": "View service",
  "services.priceLabel": "Price",
  "services.durationLabel": "Duration",
  "services.descriptionEmpty": "No description",
  "services.close": "Close",

  // ── Staff ──
  "staff.title": "Staff",
  "staff.description": "Manage team members, services and working hours.",
  "staff.addStaff": "Add staff",
  "staff.noStaff": "No staff yet",
  "staff.noStaffDesc": "Add team members so customers can book with them.",
  "staff.active": "Active",
  "staff.off": "Off",
  "staff.services": "Services ({count})",
  "staff.noServicesAssigned": "No services assigned",
  "staff.edit": "Edit",
  "staff.remove": "Remove",
  "staff.editStaff": "Edit staff",
  "staff.addStaffTitle": "Add staff",
  "staff.removeTitle": "Remove {name}?",
  "staff.removeDesc":
    "This permanently deletes the staff member. Existing bookings keep their times; the staff link is removed.",
  "staff.deleteBlocked":
    "{name} has upcoming bookings, so they cannot be deleted. Deactivate them to hide them from new bookings.",
  "staff.deleteBlockedInactive":
    "{name} has upcoming bookings, so they cannot be deleted. Deactivate does not free a plan seat — upgrade or wait until bookings are completed or cancelled.",
  "staff.deactivateInstead": "Deactivate instead",
  "staff.cancel": "Cancel",
  "staff.removed": "Staff member removed",
  "staff.updated": "Staff updated",
  "staff.added": "Staff added",
  "staff.fullName": "Full name",
  "staff.email": "Email",
  "staff.phone": "Phone",
  "staff.workingHours": "Working hours",
  "staff.workingHoursPlaceholder": "e.g. Mon–Fri · 9–18",
  "staff.activeLabel": "Active",
  "staff.availableForBookings": "Available for bookings",
  "staff.saveChanges": "Save changes",
  "staff.saving": "Saving…",
  "staff.assignServices": "Assigned services",
  "staff.assignServicesHint": "Pick which services this team member can perform.",
  "staff.noServicesYet": "Create a service first to assign it here.",
  "staff.selectedCount": "{count} selected",

  // ── Payments ──
  "payments.title": "Payments",
  "payments.description": "Track paid, unpaid and refunded transactions.",
  "payments.export": "Export",
  "payments.connectStripe": "Connect Stripe",
  "payments.collected": "Collected",
  "payments.pendingBalance": "Pending balance",
  "payments.refunds": "Refunds",
  "payments.avgTransaction": "Avg. transaction",
  "payments.stripeNotice": "Stripe & Mollie integration coming soon",
  "payments.stripeNoticeDesc":
    "Application fees per transaction will be supported on Pro and Premium plans.",
  "payments.noPayments": "No payments yet",
  "payments.noPaymentsDesc":
    "Payments appear here once customers pay or you mark a booking as paid.",
  "payments.recentTransactions": "Recent transactions",
  "payments.customer": "Customer",
  "payments.booking": "Booking",
  "payments.amount": "Amount",
  "payments.status": "Status",
  "payments.date": "Date",
  "payments.updated": "Payment updated",
  "payments.filterAll": "All",
  "payments.filterPaid": "Paid",
  "payments.filterPending": "Pending",
  "payments.filterFailed": "Failed",
  "payments.method": "Method",

  // ── Mollie Connect ──
  "mollie.title": "Mollie payments",
  "mollie.description":
    "Connect your own Mollie account to accept payments. Payouts go directly to you.",
  "mollie.connect": "Connect Mollie",
  "mollie.disconnect": "Disconnect",
  "mollie.pending": "Connection pending",
  "mollie.requestStarted": "Connection request started",
  "mollie.disconnected": "Mollie disconnected",
  "mollie.onboarding": "Onboarding",
  "mollie.feeEnabled": "Booking fee active",
  "mollie.feePercent": "Booking fee per booking",
  "mollie.feeNoFee": "No booking fee",
  "mollie.payouts": "Payouts",
  "mollie.payoutsValue": "Direct to your Mollie",
  "mollie.platformFeeNotice":
    "FlowyBookings charges a fixed booking fee per successful booking — regardless of the amount. Customer funds settle directly into your own Mollie account, never on our platform.",
  "mollie.status.not_connected": "Not connected",
  "mollie.status.pending": "Pending",
  "mollie.status.connected": "Connected",
  "mollie.status.disconnected": "Disconnected",
  "mollie.status.error": "Error",
  "mollie.onboarding.not_started": "Not started",
  "mollie.onboarding.in_review": "In review",
  "mollie.onboarding.completed": "Completed",
  "mollie.onboarding.rejected": "Rejected",
  "mollie.nudge.title": "Get paid faster — connect Mollie",
  "mollie.nudge.body":
    "Accept iDEAL, cards, and Bancontact at checkout. Funds settle directly into your own Mollie account, with deposits to reduce no-shows.",
  "mollie.nudge.benefit1": "Direct payouts to you",
  "mollie.nudge.benefit2": "Fewer no-shows with deposits",
  "mollie.nudge.benefit3": "Low platform fee",
  "mollie.nudge.cta": "Connect Mollie",
  "mollie.nudge.notifTitle": "Connect Mollie to accept payments",
  "mollie.nudge.notifMessage":
    "Your shop hasn't connected a payment provider yet. Connect Mollie to take deposits and reduce no-shows.",
  "mollie.connected": "Mollie connected successfully",
  "mollie.connectFailed": "Failed to connect Mollie",
  "mollie.notConfigured": "Mollie Connect is not configured on the platform",
  "mollie.reconnect": "Reconnect",

  // Pre-connect screen
  "mollie.preConnect.title": "Connect your payments",
  "mollie.preConnect.subtitle":
    "Connect once — then you'll receive payments directly into your own Mollie account.",
  "mollie.preConnect.benefit1": "Receive payments directly to your own account",
  "mollie.preConnect.benefit2": "Let customers pay online (iDEAL, card, Bancontact)",
  "mollie.preConnect.benefit3": "Fewer no-shows thanks to deposits",
  "mollie.preConnect.warningTitle": "Heads up — pick the right organisation in Mollie",
  "mollie.preConnect.warningBody":
    "Mollie will ask you to choose an organisation. Select only the organisation that matches:",
  "mollie.preConnect.cancel": "Cancel",
  "mollie.preConnect.continue": "Continue to Mollie",

  // Post-callback confirmation
  "mollie.confirm.title": "Payments almost active",
  "mollie.confirm.subtitle": "We've connected to:",
  "mollie.confirm.connectedWith": "We've connected to",
  "mollie.confirm.orgIdLabel": "Business ID",
  "mollie.confirm.question": "Is this your business?",
  "mollie.confirm.safetyHint": "Seeing the wrong business here? Tap 'Reconnect'.",
  "mollie.confirm.yes": "Yes, that's right",
  "mollie.confirm.no": "Reconnect",
  "mollie.confirm.confirmed": "Thanks — your payments are now active",
  "mollie.confirm.reconnecting": "Restarting the connection…",
  "mollie.confirm.pendingPill": "Quick check",
  "mollie.dashboard.activeTitle": "Payments active",
  "mollie.dashboard.connectedWith": "Connected with",

  // ── Admin: Providers ──
  "adminProviders.title": "Payment providers",
  "adminProviders.description": "Per-shop Mollie Connect status and platform fee configuration.",
  "adminProviders.empty": "No shops have started a Mollie connection yet.",
  "adminProviders.account": "Account",
  "adminProviders.fee": "Fee",
  "adminProviders.markConnected": "Mark connected",
  "adminProviders.disconnect": "Disconnect",
  "adminProviders.updated": "Provider status updated",
  "adminProviders.revenueByShop": "Revenue by shop",
  "adminProviders.revenueByShopDesc": "Top shops ranked by collected platform fees.",

  // ── Admin: Mollie health ──
  "adminMollieHealth.title": "Mollie Connect health",
  "adminMollieHealth.description":
    "Per shop: connection status, organization, token expiry, last refresh and platform fees (last 30 days).",
  "adminMollieHealth.empty": "No shops with a Mollie connection found.",
  "adminMollieHealth.searchPlaceholder": "Search by shop or organization…",
  "adminMollieHealth.counts": "{connected} of {total} connected",
  "adminMollieHealth.feesTotal30d": "Platform fees 30d: {amount}",
  "adminMollieHealth.payments": "{n} payments",
  "adminMollieHealth.connectedAt": "Connected {when}",
  "adminMollieHealth.tokenExpired": "Expired {when}",
  "adminMollieHealth.filter.all": "All",
  "adminMollieHealth.filter.issues": "Issues",
  "adminMollieHealth.filter.connected": "Connected",
  "adminMollieHealth.col.shop": "Shop",
  "adminMollieHealth.col.status": "Status",
  "adminMollieHealth.col.organization": "Organization",
  "adminMollieHealth.col.token": "Token expires",
  "adminMollieHealth.col.lastRefresh": "Last refresh",
  "adminMollieHealth.col.fees30d": "Fees (30d)",
  "adminMollieHealth.pill.error": "Error",
  "adminMollieHealth.pill.expired": "Token expired",

  "common.yes": "Yes",
  "common.no": "No",

  // ── Inbox (in-app notifications) ──
  "inbox.title": "Inbox",
  "inbox.pageDescription": "In-app messages, alerts and channel preferences.",
  "inbox.tabInbox": "Inbox",
  "inbox.tabSettings": "Settings",
  "inbox.empty": "No notifications yet.",
  "inbox.allRead": "All caught up",
  "inbox.unreadCount": "{n} unread",
  "inbox.markAllRead": "Mark all read",
  "inbox.markedAllRead": "All notifications marked as read",
  "inbox.viewAll": "View all notifications",
  "inbox.filter.all": "All",
  "inbox.filter.unread": "Unread",
  "inbox.type.system": "System",
  "inbox.type.billing": "Billing",
  "inbox.type.bookings": "Bookings",
  "inbox.type.alerts": "Alerts",
  "inbox.type.admin": "Admin",

  // ── Admin: Messages ──
  "adminMessages.title": "Messages",
  "adminMessages.description": "Send in-app notifications to one shop, several, or all shops.",
  "adminMessages.composer": "New broadcast",
  "adminMessages.type": "Type",
  "adminMessages.message": "Message",
  "adminMessages.actionUrl": "Action URL (optional)",
  "adminMessages.recipients": "Recipients",
  "adminMessages.allShops": "All shops ({n})",
  "adminMessages.selectShops": "Pick shops",
  "adminMessages.noShops": "No shops yet.",
  "adminMessages.send": "Send",
  "adminMessages.sending": "Sending…",
  "adminMessages.sentToast": "Sent to {n} shop(s)",
  "adminMessages.required": "Title and message are required",
  "adminMessages.recent": "Recent broadcasts",
  "adminMessages.recentSub": "Last messages sent from admin.",
  "adminMessages.noBroadcasts": "No broadcasts yet.",
  "adminMessages.deliveredTo": "Sent to {n}",
  "adminMessages.readBy": "{n} read",
  "adminMessages.emailTemplates": "Email templates",
  "adminMessages.emailTemplatesSub":
    "Edit notification templates and send a test to verify Resend delivery.",
  "adminMessages.preview": "Preview",
  "adminMessages.previewSubject": "Subject",
  "adminMessages.previewDescription": "Rendered from demo data — nothing is sent.",
  "adminMessages.noPreviewData": "No preview data available for this template.",
  "adminMessages.emailVars": "Allowed variables",
  "adminMessages.emailBodyHtml": "HTML body",
  "adminMessages.emailSave": "Save template",
  "adminMessages.emailSaved": "Template saved",
  "adminMessages.emailNoTemplate": "No email templates found. Run the email_templates migration.",
  "adminMessages.emailTestTo": "Send test to",
  "adminMessages.emailSendTest": "Send test",
  "adminMessages.emailTestHint": "Queues a system_test email. The worker sends it through Resend.",
  "adminMessages.emailTestRequired": "Enter a recipient email address.",
  "adminMessages.emailTestUnauth": "You are not signed in.",
  "adminMessages.emailTestFailed": "Could not queue the test email.",
  "adminMessages.emailTestQueued": "Test email queued.",
  "adminMessages.emailRecentSends": "Recent test sends",
  "adminMessages.emailNoSends": "No test sends yet.",

  // ── Notifications ──
  "notifications.title": "Notifications",
  "notifications.description": "Reminders and confirmations across channels.",
  "notifications.email": "Email",
  "notifications.emailDesc": "Always free.",
  "notifications.sms": "SMS",
  "notifications.smsDesc": "€0.04 / message.",
  "notifications.whatsapp": "WhatsApp",
  "notifications.whatsappDesc": "Coming soon.",
  "notifications.whatsappBadge": "Coming soon",
  "notifications.whatsappComingTitle": "WhatsApp reminders are on the way.",
  "notifications.whatsappComingBody":
    "We're building an official WhatsApp Business integration. For now this channel stays off on every plan — use email or SMS to send reminders.",
  "notifications.whatsappRowTitle": "WhatsApp reminders",
  "notifications.whatsappRowDesc":
    "Send automated reminders via WhatsApp Business — still in development.",
  "notifications.reminderEvents": "Reminder events",
  "notifications.reminderDesc": "Pick which messages go out automatically.",
  "notifications.confirm": "Booking confirmation",
  "notifications.confirmDesc": "Sent right after a booking is made.",
  "notifications.reminder24": "24-hour reminder",
  "notifications.reminder24Desc": "Sent 24h before the appointment.",
  "notifications.reminder2": "2-hour reminder",
  "notifications.reminder2Desc": "Final reminder, 2h before.",
  "notifications.noshow": "No-show follow-up",
  "notifications.noshowDesc": "Recover lost bookings politely.",
  "notifications.saveChanges": "Save changes",
  "notifications.saving": "Saving…",
  "notifications.saved": "Preferences saved",
  "notifications.unsaved": "Unsaved changes",
  "notifications.depositTitle": "Deposits (coming soon)",
  "notifications.depositSub":
    "Set a default deposit percent. Used to require deposits from repeat no-show customers when payments go live.",
  "notifications.depositPercent": "Default deposit %",
  "notifications.depositSaved": "Deposit settings saved",
  "automations.title": "Automation flows",
  "automations.sub": "Real emails sent automatically. Toggle per shop.",
  "automations.confirmation": "Booking confirmation",
  "automations.confirmationDesc": "Sent immediately after a booking is created.",
  "automations.reminder24": "24-hour reminder",
  "automations.reminder24Desc": "Sent ~24h before the appointment starts.",
  "automations.reminder2": "2-hour reminder",
  "automations.reminder2Desc": "Final reminder ~2h before the appointment.",
  "automations.followup": "Post-appointment follow-up",
  "automations.followupDesc": "Sent ~2h after the appointment ends.",
  "automations.saved": "Automations saved",
  "automations.statusActive": "Active · cron every 5 min",
  "automations.poweredBy": "Powered by FlowyBookings email",
  "automations.reminderSms": "Reminder SMS (2 hours)",
  "automations.reminderSmsDesc":
    "Also send an SMS ~2h before the appointment. Requires credits and a phone number.",
  "automations.smsCreditsTitle": "SMS balance",
  "automations.smsCreditsBalance": "Remaining balance",
  "automations.smsCreditsUsed": "Used",
  "automations.smsCreditsFree": "Granted free",
  "automations.smsCreditsHint": "Each reminder SMS uses 1 credit.",
  "automations.smsCreditsLow":
    "Low balance — reminders will be skipped once the balance reaches 0.",
  "automations.smsCreditsEmpty":
    "No credits — SMS reminders are disabled until new credits are added.",
  "automations.smsProviderPending": "Sending will be activated once the SMS provider is connected.",
  "automations.smsCreditsTopUp": "Top up SMS credits",

  "smsTopup.title": "Top up SMS credits",
  "smsTopup.subtitle": "One-off payment via Mollie. Credits are added immediately after payment.",
  "smsTopup.popular": "Popular",
  "smsTopup.bestDeal": "Best deal",
  "smsTopup.credits": "credits",
  "smsTopup.perCredit": "per SMS",
  "smsTopup.choose": "Choose this pack",
  "smsTopup.cancel": "Cancel",
  "smsTopup.processing": "Redirecting…",
  "smsTopup.successReturn":
    "Your payment was processed. Your balance will update in a few seconds.",
  "smsTopup.failedReturn": "Payment failed or cancelled. Please try again.",
  "smsTopup.errorGeneric": "Could not start payment. Please try again.",

  // ── Settings ──
  "settings.title": "Settings",
  "settings.description": "Brand, hours, booking rules and timezone.",
  "settings.saveChanges": "Save changes",
  "settings.subscription": "Your subscription",
  "settings.currentPlan": "Current plan",
  "settings.planActive": "Active",
  "settings.planTrial": "Trial",
  "settings.planExpired": "Expired",
  "settings.nextPayment": "Next billing date",
  "settings.changePlan": "Change plan",
  "settings.cancelPlan": "Cancel",
  "settings.daysLeft": "{n} days remaining",
  "settings.customerPayments": "Customer payments",
  "settings.businessInfo": "Business info",
  "settings.shopSlug": "Booking URL",
  "settings.shopSlugHint": "Cannot be changed after creation",
  "settings.city": "City",
  "settings.openingHoursTitle": "Opening hours",
  "settings.stripeTitle": "Stripe (coming soon)",
  "settings.stripeDesc": "135+ currencies, worldwide. Best choice for international.",
  "settings.stripeComingSoon": "Coming soon",
  "settings.upgradeFee": "Upgrade to Premium for 0.5% fee",
  "settings.platformFee": "Platform fee: {pct}%",
  "settings.mollieManageTitle": "Manage Mollie payments",
  "settings.mollieManageDesc": "Connect or manage your Mollie account on the Payments page.",
  "settings.mollieManageCta": "Go to Payments →",
  "settings.shopProfile": "Shop profile",
  "settings.shopName": "Shop name",
  "settings.phone": "Phone",
  "settings.email": "Email",
  "settings.timezone": "Timezone",
  "settings.address": "Address",
  "settings.branding": "Branding",
  "settings.uploadLogo": "Upload logo",
  "settings.logoHint": "PNG or SVG · max 2MB.",
  "settings.brandColor": "Brand color",
  "settings.businessHours": "Business hours",
  "settings.businessHoursMissingTitle": "Business hours are not set",
  "settings.businessHoursMissingBody":
    "Online bookings stay unavailable until you save opening hours. Review the schedule below and click Save.",
  "settings.closed": "Closed",
  "settings.bookingRules": "Booking rules",
  "settings.minNotice": "Min. notice (hours)",
  "settings.maxWindow": "Max booking window (days)",
  "settings.slotInterval": "Slot interval (min)",
  "settings.defaultDeposit": "Default deposit %",
  "settings.monday": "Monday",
  "settings.tuesday": "Tuesday",
  "settings.wednesday": "Wednesday",
  "settings.thursday": "Thursday",
  "settings.friday": "Friday",
  "settings.saturday": "Saturday",
  "settings.sunday": "Sunday",
  "settings.saved": "Settings saved",
  "settings.saving": "Saving…",
  "settings.markOpen": "Open",
  "settings.markClosed": "Close",
  "settings.logoUploaderHint": "PNG, JPG, WEBP or SVG · max 2MB",
  "settings.logoUploadingShort": "Working…",
  "settings.logoReplace": "Replace",
  "settings.logoUpload": "Upload logo",
  "settings.logoRemove": "Remove",
  "settings.logoTooLarge": "Logo is too large (max 2MB)",
  "settings.logoUploaded": "Logo uploaded",
  "settings.logoRemoved": "Logo removed",
  "settings.trialExpiredTitle": "Your trial has expired",
  "settings.trialExpiredBody":
    'Pick a plan to continue. New bookings via your public page and the "New appointment" button are temporarily blocked.',
  "settings.trialExpiredCta": "Pick a plan",

  // ── Analytics ──
  "analytics.title": "Analytics",
  "analytics.description": "Trends, revenue, retention and what's working.",
  "analytics.revenue30d": "Revenue (30d)",
  "analytics.bookings30d": "Bookings (30d)",
  "analytics.returningCustomers": "Returning customers",
  "analytics.noShowRate": "No-show rate",
  "analytics.monthlyRevenue": "Monthly revenue",
  "analytics.trailing8months": "Trailing 8 months",
  "analytics.bookingsByDay": "Bookings by day",
  "analytics.thisWeek": "This week",
  "analytics.topServices": "Top services",
  "analytics.topServicesSub": "Most booked in the last 30 days",
  "analytics.bookings": "{n} bookings",
  "analytics.noData": "No data to show yet. Create your first bookings to see analytics.",
  "analytics.basedOn": "Based on {bookings} bookings, {payments} payments, {customers} customers.",

  // ── Booking flow ──
  "book.title": "Book an appointment",
  "book.secureBooking": "Secure booking · powered by FlowyBookings",
  "book.stepShop": "Shop",
  "book.stepService": "Service",
  "book.stepStaff": "Staff",
  "book.stepDateTime": "Date & time",
  "book.stepDetails": "Your details",
  "book.stepReview": "Review",
  "book.chooseShop": "Choose a shop",
  "book.chooseShopSub": "Pick where you'd like to book.",
  "book.chooseService": "Choose a service",
  "book.chooseServiceSub": "What can we book for you?",
  "book.chooseStaff": "Choose a team member",
  "book.chooseStaffSub": "Or pick anyone available.",
  "book.anyAvailable": "Any available",
  "book.firstOpenSlot": "First open slot",
  "book.pickDate": "Pick a date & time",
  "book.pickDateSub": "Pick a date first, then choose a time slot.",
  "book.pickDateFirst": "Pick a date first to see available times.",
  "book.shopClosed": "The shop is closed on this day.",
  "book.slotBusy": "This slot is taken",
  "book.availableTimes": "Available times",
  "book.free": "Free",
  "book.errName": "Enter a valid name (min. 2 characters).",
  "book.errPhone": "Enter a valid phone number.",
  "book.errEmail": "Enter a valid email address.",
  "book.payNow": "Pay {amount} now",
  "book.confirmFree": "Confirm appointment",
  "book.freeNotice": "No payment required — you'll receive a confirmation by email immediately.",
  "book.depositBadge": "{amount} deposit",
  "book.noDepositBadge": "No deposit",
  "book.dueNow": "Due now",
  "book.autoAssigned": "auto-assigned",
  "book.yourDetails": "Your details",
  "book.yourDetailsSub": "We'll send your confirmation here.",
  "book.fullName": "Full name",
  "book.phone": "Phone",
  "book.email": "Email",
  "book.noteOptional": "Note (optional)",
  "book.notePlaceholder": "Anything we should know?",
  "book.reviewConfirm": "Review & confirm",
  "book.reviewSub": "Almost done — review your booking.",
  "book.shop": "Shop",
  "book.service": "Service",
  "book.with": "With",
  "book.when": "When",
  "book.customerLabel": "Customer",
  "book.durationLabel": "Duration",
  "book.price": "Price",
  "book.depositDue": "Deposit due now",
  "book.remainingAtAppointment": "Remaining at appointment",
  "book.depositNotice":
    "You'll pay a {deposit} deposit now to confirm this booking — the remaining {remaining} is due at your appointment.",
  "book.back": "Back",
  "book.continue": "Continue",
  "book.confirmBooking": "Confirm booking",
  "book.summary": "Summary",
  "book.total": "Total",
  "book.deposit": "Deposit",
  "book.date": "Date",
  "book.time": "Time",
  "book.slotTaken": "This time slot was just taken. Please pick another.",
  "book.failed": "Booking failed. Please try again.",
  "book.youreBooked": "You're booked in!",
  "book.confirmationSentSub":
    "A confirmation has been sent to your email. Reminders will follow 24h and 2h before the appointment.",
  "book.bookAnother": "Book another",
  "book.addToCalendar": "Add to calendar",
  "book.backHome": "Back to home",
  "book.bookingRef": "Reference",
  "book.notFound": "Booking not found",
  "book.notFoundSub": "This booking reference does not exist or has expired.",
  "book.confirmingPayment": "Confirming your payment…",
  "book.confirmingPaymentSlow": "This is taking longer than expected",
  "book.confirmingPaymentSlowSub":
    "We're still processing your payment. You'll receive a confirmation by email once it's done.",
  "book.paymentFailedTitle": "Payment failed",
  "book.paymentFailedSub": "Your payment didn't go through or the booking was cancelled. Please try again.",
  "book.tryAgain": "Try again",
  "book.linkNotFound": "Booking link not found",
  "book.linkNotFoundSub": "This link doesn't match any shop. Check the URL or contact the business directly.",
  "book.shopInactive": "This shop is not available",
  "book.shopInactiveSub": "{shop} is not accepting online bookings at the moment.",
  "book.shopInactiveSubGeneric": "This shop is not accepting online bookings at the moment.",
  "book.shopUnavailable": "Bookings temporarily unavailable",
  "book.shopUnavailableSub": "{shop} is temporarily not accepting new appointments. Please try again later.",
  "book.noServices": "No services found for this shop",
  "book.noServicesSub": "Please check back later or contact the shop.",
  "book.serviceUnavailable": "Temporarily unavailable",
  "book.mollieNotConnected":
    "This shop can't accept deposit payments right now. Please try again later or contact the shop.",
  "book.noStaff": "No team members found for this shop",
  "book.noStaffSub": "Please check back later or contact the shop.",
  "book.noTimes": "No available times on this day.",

  // ── Booking confirmation ──
  "confirmation.title": "You're booked in!",
  "confirmation.sub":
    "A confirmation has been sent to your email. We've also scheduled a 24h and 2h reminder.",
  "confirmation.bookAnother": "Book another",
  "confirmation.backHome": "Back to home",

  // ── Onboarding ──
  "onboarding.createShop": "Create your shop",
  "onboarding.sub": "Let's set up your workspace. You can change everything later.",
  "onboarding.shopName": "Shop name",
  "onboarding.shopNamePlaceholder": "e.g. Inkwell Studio",
  "onboarding.urlSlug": "URL slug",
  "onboarding.slugHint": "Only lowercase letters, numbers and dashes.",
  "onboarding.createBtn": "Create shop",
  "onboarding.created": "Shop created — welcome aboard!",
  "onboarding.category": "Business type",
  "onboarding.categoryPlaceholder": "Select a category",
  "onboarding.categoryRequired": "Please select a business type.",
  "onboarding.categoryHair": "Hair",
  "onboarding.categoryBarber": "Barber",
  "onboarding.categoryNails": "Nails",
  "onboarding.categoryBeauty": "Beauty",
  "onboarding.categoryTattoo": "Tattoo",
  "onboarding.categoryPet": "Pet grooming",
  "onboarding.timezone": "Timezone",
  "onboarding.timezoneHint":
    "Booking times use this shop clock. Prefills from your device — change if the salon is elsewhere.",
  "onboarding.timezoneRequired": "Please select a timezone.",

  // ── Empty states ──
  "empty.noShop": "No shop selected",
  "empty.noShopDesc": "Pick a shop from the dropdown in the top-right to view its data.",

  // ── Statuses ──
  "status.pending": "Pending",
  "status.payment_pending": "Awaiting payment",
  "status.confirmation_pending": "Needs confirmation",
  "status.confirmed": "Confirmed",
  "status.completed": "Completed",
  "status.cancelled": "Cancelled",
  "status.no_show": "No-show",
  "status.no-show": "No-show",
  "status.paid": "Paid",
  "status.unpaid": "Unpaid",
  "status.deposit_paid": "Deposit paid",
  "status.refunded": "Refunded",
  "status.partial": "Partial",
  "status.failed": "Failed",
  "status.active": "Active",
  "status.suspended": "Suspended",

  // ── Admin overview ──
  "adminOverview.title": "Platform overview",
  "adminOverview.description": "The health of FlowyBookings across all shops.",
  "adminOverview.totalShops": "Total shops",
  "adminOverview.totalUsers": "Total users",
  "adminOverview.totalBookings": "Total bookings",
  "adminOverview.totalRevenue": "Total revenue",
  "adminOverview.fees": "{amount} fees",
  "adminOverview.active": "{n} active",
  "adminOverview.topShops": "Top shops by revenue",
  "adminOverview.mostActiveShops": "Most active shops",
  "adminOverview.mostActiveShopsSub": "Last 30 days · by booking count",
  "adminOverview.bookingsCount": "{n} bookings",
  "adminOverview.noActiveShops": "No activity in the last 30 days.",
  "adminOverview.noShops": "No shops yet.",
  "adminOverview.needsAttention": "Needs attention",
  "adminOverview.failedPayments": "Failed payments",
  "adminOverview.pendingApprovals": "Pending shop approvals",
  "adminOverview.suspendedShops": "Suspended shops",
  "adminOverview.noShowsLabel": "No-shows",
  "adminOverview.recentBookings": "Recent bookings",
  "adminOverview.noBookings": "No bookings yet.",
  "adminOverview.walkIn": "Walk-in",

  // ── Admin shops ──
  "adminShops.title": "Shops",
  "adminShops.description": "Approve, suspend and oversee every shop on the platform.",
  "adminShops.searchPlaceholder": "Search shops…",
  "adminShops.all": "all",
  "adminShops.shop": "Shop",
  "adminShops.owner": "Owner",
  "adminShops.plan": "Plan",
  "adminShops.status": "Status",
  "adminShops.bookings": "Bookings",
  "adminShops.revenue": "Revenue",
  "adminShops.created": "Created",
  "adminShops.policyAccepted": "Policy accepted",
  "adminShops.policyMissing": "Not accepted",
  "adminShops.policyAcceptedOn": "Policy accepted on",
  "adminShops.policyNotAcceptedWarning": "Policy not yet accepted",
  "adminShops.policyFilter.all": "All",
  "adminShops.policyFilter.accepted": "Policy ✅",
  "adminShops.policyFilter.missing": "Policy ❌",
  "adminShops.noShops": "No shops found.",
  "adminShops.updated": "Shop status updated",
  "adminShops.customersStat": "Customers: {total} (of which {imported} imported)",
  "adminShops.lastLogin": "Last login",
  "adminShops.activity": "Activity",
  "adminShops.activityActive": "Active",
  "adminShops.activityInactive": "Inactive 14d+",
  "adminBell.aria": "Platform activity",
  "adminShopDetail.back": "← Back to shops",
  "adminShopDetail.loginAs": "Log in as shop",
  "adminShopDetail.suspend": "Suspend",
  "adminShopDetail.activate": "Activate",
  "adminShopDetail.contact": "Contact",
  "adminShopDetail.owner": "Owner",
  "adminShopDetail.subscription": "Subscription",
  "adminShopDetail.changePlan": "Change plan",
  "adminShopDetail.expiresAt": "Expires on",
  "adminShopDetail.billingCycle": "Billing cycle",
  "adminShopDetail.nextBilling": "Next billing date",
  "adminShopDetail.statBookings": "Total bookings",
  "adminShopDetail.statBookingsMonth": "Bookings this month",
  "adminShopDetail.statCustomers": "Customers",
  "adminShopDetail.statRevenue": "Revenue",
  "adminShopDetail.statFees": "Platform fees",
  "adminShopDetail.statServices": "Services",
  "adminShopDetail.statStaff": "Active staff",
  "adminShopDetail.statFailedPayments": "Failed payments",
  "adminShopDetail.customerSources": "Customer sources",
  "adminShopDetail.noCustomers": "No customers yet.",
  "adminShopDetail.recentActivity": "Recent activity",
  "adminShopDetail.noActivity": "No activity for this shop yet.",
  "adminShopDetail.policyStatus": "Policy status",
  "adminShopDetail.adminNotes": "Admin notes",
  "adminShopDetail.notFound": "Shop not found.",
  "adminShopDetail.viewDetail": "Open detail",

  // ── Admin users ──
  "adminUsers.title": "Users",
  "adminUsers.description": "All platform accounts — owners, staff, admins and customers.",
  "adminUsers.searchPlaceholder": "Search users…",
  "adminUsers.all": "All",
  "adminUsers.user": "User",
  "adminUsers.roles": "Roles",
  "adminUsers.linkedShops": "Linked shops",
  "adminUsers.joined": "Joined",
  "adminUsers.noRole": "No role",
  "adminUsers.noUsers": "No users found.",
  "adminUsers.roleRemoved": "Role removed",

  // ── Admin bookings ──
  "adminBookings.title": "All bookings",
  "adminBookings.description":
    "Investigate disputes, no-shows and cancellations across every shop.",
  "adminBookings.searchPlaceholder": "Search bookings…",
  "adminBookings.booking": "Booking",
  "adminBookings.shop": "Shop",
  "adminBookings.when": "When",
  "adminBookings.status": "Status",
  "adminBookings.payment": "Payment",
  "adminBookings.total": "Total",
  "adminBookings.noBookings": "No bookings found.",
  "adminBookings.walkIn": "Walk-in",

  // ── Admin payments ──
  "adminPayments.title": "Payments & revenue",
  "adminPayments.description": "Transaction revenue, platform fees and refunds across all shops.",
  "adminPayments.totalRevenue": "Total revenue",
  "adminPayments.platformFees": "Platform fees",
  "adminPayments.totalTransactions": "Total transactions",
  "adminPayments.refunds": "Refunds",
  "adminPayments.events": "{n} events",
  "adminPayments.allTransactions": "All transactions",
  "adminPayments.shop": "Shop",
  "adminPayments.amount": "Amount",
  "adminPayments.fee": "Fee",
  "adminPayments.status": "Status",
  "adminPayments.provider": "Provider",
  "adminPayments.date": "Date",
  "adminPayments.noPayments": "No payments yet.",

  // ── Admin logs ──
  "adminLogs.title": "Activity log",
  "adminLogs.description": "Recent platform activity across shops, bookings, payments and users.",
  "adminLogs.noActivity": "No activity yet.",

  // ── Admin plans ──
  "adminPlans.title": "Subscription plans",
  "adminPlans.description": "Pricing tiers, feature access and transaction fees.",
  "adminPlans.newPlan": "New plan",
  "adminPlans.mostPopular": "Most popular",
  "adminPlans.transactionFee": "transaction fee",
  "adminPlans.edit": "Edit",
  "adminPlans.archive": "Archive",

  // ── Admin support ──
  "adminSupport.title": "Support tickets",
  "adminSupport.description": "Shop and payment issues — assigned and unassigned.",
  "adminSupport.newTicket": "New ticket",
  "adminSupport.subject": "Subject",
  "adminSupport.shop": "Shop",
  "adminSupport.priority": "Priority",
  "adminSupport.status": "Status",
  "adminSupport.updated": "Updated",
  "adminSupport.open": "Open",

  // ── Admin settings ──
  "adminSettings.title": "Platform settings",
  "adminSettings.description": "Branding, templates and global defaults.",
  "adminSettings.saveChanges": "Save changes",
  "adminSettings.branding": "Branding",
  "adminSettings.platformName": "Platform name",
  "adminSettings.supportEmail": "Support email",
  "adminSettings.brandColor": "Brand color",
  "adminSettings.bookingDefaults": "Booking defaults",
  "adminSettings.slotInterval": "Default slot interval (min)",
  "adminSettings.minNotice": "Min notice (hours)",
  "adminSettings.maxWindow": "Max booking window (days)",
  "adminSettings.emailTemplates": "Email templates",
  "adminSettings.editTemplate": "Edit template",
  "adminSettings.legal": "Legal",
  "adminSettings.termsOfService": "Terms of service",
  "adminSettings.privacyPolicy": "Privacy policy",
  "adminSettings.acceptableUse": "Acceptable use",
  "adminSettings.dangerZone": "Danger zone",
  "adminSettings.dangerDesc": "Maintenance mode, data export, account deletion.",
  "adminSettings.enableMaintenance": "Enable maintenance",
  "adminSettings.exportData": "Export all data",

  // ── Platform billing (admin Mollie for FlowyBookings subscriptions) ──
  "platformBilling.title": "Subscription Billing Provider",
  "platformBilling.subtitle": "Manage FlowyBookings' own Mollie connection for plan payments",
  "platformBilling.adminOnly": "Hidden admin only — never visible to shop owners",
  "platformBilling.sectionA": "Subscription Billing",
  "platformBilling.sectionADesc":
    "FlowyBookings' own Mollie account for Starter / Pro / Premium plan payments. Configured here.",
  "platformBilling.sectionB": "Booking Payment Providers",
  "platformBilling.sectionBDesc":
    "Each shop's own Mollie Connect account for booking deposits. Managed per shop, overviewed under Providers.",
  "platformBilling.runCheck": "Run health check",
  "platformBilling.readiness": "Platform billing readiness",
  "platformBilling.statusReady": "Ready",
  "platformBilling.statusMissing": "{n} missing",
  "platformBilling.checklist.apiKey": "Platform Mollie API key configured",
  "platformBilling.checklist.webhook": "Billing webhook configured",
  "platformBilling.checklist.flowConnected": "Plan payment flow connected",
  "platformBilling.checklist.lifecycleActive": "Subscription lifecycle active",
  "platformBilling.checklist.adminPage": "Admin billing page connected",
  "platformBilling.field.mode": "Mode",
  "platformBilling.field.apiKey": "API key",
  "platformBilling.field.clientId": "Client ID",
  "platformBilling.field.clientSecret": "Client secret",
  "platformBilling.field.webhook": "Webhook URL",
  "platformBilling.mode.test": "Test",
  "platformBilling.mode.live": "Live",
  "platformBilling.mode.unknown": "Unknown",
  "platformBilling.mode.missing": "Not set",
  "platformBilling.present": "Present",
  "platformBilling.absent": "Absent",
  "platformBilling.notConfigured": "Not configured",
  "platformBilling.copied": "Copied",
  "platformBilling.lastWebhook": "Last webhook",
  "platformBilling.lastTest": "Last test result",
  "platformBilling.lastError": "Last known billing error",
  "platformBilling.manageSecrets": "Manage Mollie keys",
  "platformBilling.manageSecretsTitle": "Manage Mollie subscription keys",
  "platformBilling.manageSecretsIntro":
    "These secrets power FlowyBookings' own subscription billing. They are stored securely in Lovable Secrets and are never visible in the admin UI. To add or rotate a value, open Lovable Cloud → Settings → Secrets and use the secret names below.",
  "platformBilling.openSecrets": "Open secrets manager",
  "platformBilling.copyName": "Copy name",
  "platformBilling.editMode": "Edit mode",
  "platformBilling.saveMode": "Save",
  "platformBilling.cancel": "Cancel",
  "platformBilling.modeSaved": "Mode saved",
  "platformBilling.lastUpdated": "Last updated",
  "platformBilling.required": "Required",
  "platformBilling.optional": "Optional (Mollie Connect — not used by subscription billing yet)",
  "platformBilling.lastChecked": "Last checked",

  // ── Booking confirmation templates ──
  "emailTemplate.bookingConfirmation": "Booking confirmation",
  "emailTemplate.reminder24": "24-hour reminder",
  "emailTemplate.reminder2": "2-hour reminder",
  "emailTemplate.noShowFollowUp": "No-show follow-up",
  "emailTemplate.refundIssued": "Refund issued",

  // ── Language ──
  "lang.nl": "NL",
  "lang.en": "EN",

  // ── Onboarding (multi-step extras) ──
  "onboarding.stepWelcome": "Welcome",
  "onboarding.stepShop": "Your shop",
  "onboarding.stepReady": "You're ready",
  "onboarding.progress": "Step {current} of {total}",
  "onboarding.welcomeTitle": "Welcome to FlowyBookings 👋",
  "onboarding.welcomeSub": "Let's get your shop live in under a minute. You'll get:",
  "onboarding.benefit1": "A branded booking page customers can use 24/7",
  "onboarding.benefit2": "Automatic email reminders to cut no-shows",
  "onboarding.benefit3": "A live calendar synced across your team",
  "onboarding.benefit4": "Insights into revenue, retention and busy hours",
  "onboarding.startBtn": "Let's go →",
  "onboarding.readyTitle": "You're all set 🎉",
  "onboarding.readySub": "Your shop is live. Here's what to do next:",
  "onboarding.next1": "Add your services and prices",
  "onboarding.next2": "Add your team members",
  "onboarding.next3": "Share your booking link",
  "onboarding.goDashboard": "Go to dashboard",
  "onboarding.shareLink": "Share your booking link",
  "onboarding.linkCopied": "Link copied to clipboard",

  // ── Upgrade / Pricing (in-app) ──
  "upgrade.pageTitle": "Upgrade your plan",
  "upgrade.pageSub": "Get more bookings, fewer no-shows, happier customers.",
  "upgrade.currentPlan": "Current plan",
  "upgrade.mostPopular": "Most popular",
  "upgrade.bestValue": "Best value",
  "upgrade.perMonth": "/month",
  "upgrade.perYear": "/year",
  "upgrade.cycle.monthly": "Monthly",
  "upgrade.cycle.yearly": "Yearly",
  "upgrade.cycle.save": "−2 mo",
  "upgrade.cycle.savingHint": "Save {months} months when billed yearly",
  "upgrade.basic": "Starter",
  "upgrade.trialLabel": "Trial",
  "upgrade.basicTagline": "Everything you need to get going",
  "upgrade.pro": "Pro",
  "upgrade.proTagline": "Grow faster with automation",
  "upgrade.premium": "Premium",
  "upgrade.premiumTagline": "Scale your business",
  "upgrade.outcomeBookings": "Up to 32% more bookings",
  "upgrade.outcomeNoShows": "Up to 71% fewer no-shows",
  "upgrade.outcomeRevenue": "Average +€840/mo extra revenue",
  "upgrade.cta.starter": "Current plan",
  "upgrade.cta.upgrade": "Upgrade to {plan}",
  "upgrade.cta.contact": "Contact sales",
  "upgrade.feat.bookings": "Unlimited bookings",
  "upgrade.feat.staff3": "Up to 3 staff",
  "upgrade.feat.staff10": "Up to 10 staff",
  "upgrade.feat.staffUnlimited": "Unlimited staff",
  "upgrade.feat.email": "Email reminders",
  "upgrade.feat.sms": "SMS reminders",
  "upgrade.feat.whatsapp": "WhatsApp reminders",
  "upgrade.feat.deposits": "Deposits & no-show protection",
  "upgrade.feat.analytics": "Basic analytics",
  "upgrade.feat.advAnalytics": "Advanced analytics & exports",
  "upgrade.feat.branding": "Custom branding",
  "upgrade.feat.multiloc": "Multi-location",
  "upgrade.feat.priority": "Priority support",
  "upgrade.feat.api": "API access",
  "upgrade.guarantee": "Active immediately after upgrade · No setup fees · Cancel anytime",
  "upgrade.startTrial": "Start 14-day free trial",
  "upgrade.toastSoon": "Checkout flow coming soon — your interest is logged.",
  "upgrade.compareTitle": "Why shops upgrade",
  "upgrade.faqTitle": "Common questions",
  "upgrade.faq1q": "Can I switch plans later?",
  "upgrade.faq1a":
    "Yes — upgrades go live immediately, downgrades take effect at the end of your current billing cycle. No data or booking loss.",
  "upgrade.faq2q": "What happens to my data?",
  "upgrade.faq2a": "Your bookings, customers and history stay safe on every plan.",
  "upgrade.faq3q": "Is there a setup fee?",
  "upgrade.faq3a": "Never. No setup fees, no hidden costs. Cancel anytime.",

  // ── Upgrade nudges (in-dashboard triggers) ──
  "nudge.noShowsTitle": "No-shows are eating into your revenue",
  "nudge.noShowsBody":
    "{count} no-shows in the last 7 days. Pro adds automatic deposits and SMS reminders that cut no-shows by up to 71%.",
  "nudge.noShowsCta": "See how Pro helps",
  "nudge.staffLimitTitle": "You've reached your staff limit",
  "nudge.staffLimitBody": "Your plan supports {limit} staff. Upgrade to add the whole team.",
  "nudge.staffLimitCta": "Upgrade plan",
  "nudge.premiumLockedTitle": "This is a {plan} feature",
  "nudge.premiumLockedBody": "Unlock {feature} and more on the {plan} plan.",
  "nudge.premiumLockedCta": "Unlock {plan}",
  "nudge.dismiss": "Dismiss",
  "nudge.lockedBadge": "Pro feature",
  "nudge.premiumBadge": "Premium",

  // ── Auth: demo gating ──
  "auth.demoDisabled": "Demo accounts are currently disabled by the platform admin.",

  // ── Admin demo controls ──
  "adminDemo.title": "Demo controls",
  "adminDemo.description":
    "Toggle demo mode, demo logins, public booking on demo shops and seeded data visibility.",
  "adminDemo.flagsTitle": "Platform flags",
  "adminDemo.modeTitle": "Demo mode",
  "adminDemo.modeDesc": "Master switch. When off, all demo behavior is hidden.",
  "adminDemo.loginsTitle": "Demo logins",
  "adminDemo.loginsDesc": "Show demo accounts on the login page.",
  "adminDemo.publicBookingTitle": "Public booking on demo shops",
  "adminDemo.publicBookingDesc": "Allow anonymous visitors to book on shops marked as demo.",
  "adminDemo.dataTitle": "Seeded demo data visibility",
  "adminDemo.dataDesc": "Show seeded demo shops in public lists.",
  "adminDemo.shopsTitle": "Mark shops as demo",
  "adminDemo.shopsDesc": "Demo shops are gated by the toggles above.",
  "adminDemo.demoBadge": "Demo",
  "adminDemo.noShops": "No shops yet.",
  "adminDemo.reseedTitle": "Reseed demo accounts",
  "adminDemo.reseedDesc":
    "Recreates the three demo users (super admin, owner, staff) with the standard password.",
  "adminDemo.reseedCta": "Reseed demo users",
  "adminDemo.saved": "Settings saved",
  "adminDemo.shopUpdated": "Shop updated",
  "adminDemo.reseeded": "Demo users reseeded",
  "adminDemo.warningTitle": "Demo mode is OFF",
  "adminDemo.warningDesc":
    "Demo accounts are blocked and demo shops are hidden from public booking.",
  // ── Plan / upgrade extras ──
  "upgrade.youAreOn": "You're on",
  "upgrade.wasOn": "Your previous plan",
  "upgrade.inactive": "Inactive",
  "upgrade.previousPlanBadge": "Previous plan",
  "upgrade.scheduledBadge": "Scheduled",
  "upgrade.toastApplied": "Plan changed to {plan}",
  "upgrade.toastUpgraded": "Your upgrade to {plan} is active immediately",
  "upgrade.toastDowngradeScheduled":
    "Downgrade scheduled — your {plan} stays active until your next billing date",
  "upgrade.cta.downgrade": "Switch to {plan}",
  "upgrade.cta.upgradeShort": "Start with {plan} →",
  "upgrade.cta.upgradePremium": "Upgrade to Premium →",
  "upgrade.cta.resubscribe": "Resubscribe to {plan} →",
  "upgrade.cta.switchToYearly": "Switch to yearly billing →",
  "upgrade.cta.switchToMonthly": "Switch to monthly at renewal",
  "upgrade.confirmDowngrade":
    "Downgrade to {plan}? Your current plan stays active until your next billing date — the change takes effect after that.",
  "upgrade.confirmUpgradeFromTrial":
    "Your upgrade starts immediately and your trial will end. You'll be redirected to Mollie's checkout page to complete payment. Continue?",
  "upgrade.confirmCycleUpgrade":
    "Switch to yearly billing now? You'll be charged the full annual price immediately, and redirected to Mollie's checkout page to complete payment.",
  "upgrade.confirmCycleDowngrade":
    "Switch to monthly billing? You'll keep yearly billing until your current term ends, then switch to monthly.",
  "upgrade.confirmCheckout":
    "You'll be redirected to Mollie's checkout page to complete your payment for {plan}. Continue?",
  "upgrade.confirmTitle": "Confirm change",
  "upgrade.confirmProceed": "Confirm",
  "upgrade.confirmDismiss": "Cancel",
  "upgrade.billingNotice":
    "Payments are securely processed by Mollie · iDEAL, credit card & Bancontact",
  "upgrade.instantNote": "Active immediately after upgrade · No interruption to bookings or data",

  // ── Admin plans extras ──
  "adminPlans.shopUpdated": "Shop plan updated",
  "adminPlans.shopsOnPlan": "shops",
  "adminPlans.manageShops": "Per-shop plan",
  "adminPlans.manageShopsDesc": "Change any shop's plan. Updates take effect immediately.",
  "adminPlans.recentChanges": "Recent plan changes",
  "adminPlans.noChanges": "No plan changes yet.",
  "adminPlans.noShops": "No shops yet.",
  "adminShops.planUpdated": "Plan updated",

  // ── Permissions ──
  "perm.staffNoBillingTitle": "Staff cannot manage billing",
  "perm.staffNoBillingDesc": "Ask the shop owner to upgrade or change the plan.",
  "perm.staffNoAccessTitle": "Owner-only area",
  "perm.staffNoAccessDesc": "This section is reserved for the shop owner.",

  // ── Admin: Billing ──
  "adminNav.billing": "Billing",
  "adminBilling.title": "Platform billing",
  "adminBilling.description": "Manage shop subscriptions, expiry dates and manual overrides.",
  "adminBilling.payingShops": "Paying shops",
  "adminBilling.subscriptionRevenue": "Subscription revenue",
  "adminBilling.expiringSoon": "Expiring soon",
  "adminBilling.next7days": "Next 7 days",
  "adminBilling.expired": "Expired",
  "adminBilling.shopsTitle": "Shops & subscriptions",
  "adminBilling.shopsDesc": "Override plans, extend trials, inspect payment status.",
  "adminBilling.empty": "No shops match this filter.",
  "adminBilling.expiresAt": "Expires",
  "adminBilling.lastPayment": "Last payment",
  "adminBilling.totalPaid": "Total paid",
  "adminBilling.extend14": "+14 days",
  "adminBilling.planUpdated": "Plan updated",
  "adminBilling.trialExtended": "Trial extended by 14 days",
  "adminBilling.logTitle": "Recent billing events",
  "adminBilling.logDesc": "Subscription activations, payment failures and manual overrides.",
  "adminBilling.noLog": "No billing events yet.",
  "adminBilling.filter.all": "All",
  "adminBilling.filter.active": "Active",
  "adminBilling.filter.expiring": "Expiring",
  "adminBilling.filter.expired": "Expired",
  "adminBilling.mrr": "Monthly recurring",
  "adminBilling.conversion": "trial→paid",
  "adminBilling.paymentHealth": "Payment health",
  "adminBilling.failed": "failed",
  "adminBilling.pending": "pending",
  "adminBilling.revenueTrend": "Revenue trend",
  "adminBilling.revenueTrendDesc": "Subscription revenue per month.",
  "adminBilling.last12mo": "Last 12 months",
  "adminBilling.planDistribution": "Plan distribution",
  "adminBilling.planDistributionDesc": "How shops are spread across tiers.",
  "adminBilling.expiryUpdated": "Expiry date updated",
  "adminBilling.grant90": "Grant 90d",
  "adminBilling.notesFor": "Admin notes ·",
  "adminBilling.notesDesc": "Internal context only — not visible to the shop owner.",
  "adminBilling.notesPlaceholder":
    "Add billing exceptions, support context, manual override reasons…",
  "adminBilling.noteSaved": "Note saved",
  "adminBilling.runSweep": "Run expire sweep now",
  "adminBilling.runningSweep": "Running sweep…",
  "adminBilling.sweepDone": "Sweep complete: {expired} expired to Starter, {pending} pending plan(s) applied",
  "adminBilling.sweepConfirmTitle": "Run expire sweep?",
  "adminBilling.sweepConfirmBody":
    "{count} shop(s) currently have an expired paid plan. Running the sweep will downgrade them to Starter, log the change, and notify each owner.",
  "adminBilling.sweepConfirmNone":
    "No shops are currently past their expiry date. Running the sweep is safe — it will simply confirm there is nothing to downgrade.",
  "adminBilling.sweepConfirmRun": "Run sweep",
  "adminBilling.sweepConfirmCancel": "Cancel",
  "adminPayments.searchShop": "Search shop…",
  "adminPayments.allPlans": "All plans",

  // ── Shop billing card ──
  "shopBilling.title": "Your subscription",
  "shopBilling.currentPlan": "Current plan",
  "shopBilling.previousPlan": "Previous plan",
  "shopBilling.renewsOn": "Renews on",
  "shopBilling.chargesOn": "Next charge",
  "shopBilling.accessUntil": "Access until",
  "shopBilling.expiredOn": "Expired on",
  "shopBilling.noExpiry": "No active subscription yet",
  "shopBilling.lapsedNoSubscription": "No active subscription — resubscribe to continue",
  "shopBilling.cycle": "Billing cycle",
  "shopBilling.history": "Payment history",
  "shopBilling.noHistory": "No subscription payments yet.",
  "shopBilling.payNow": "Continue to checkout",
  "shopBilling.mockPay": "Confirm test payment",
  "shopBilling.mockFail": "Simulate failure",
  "shopBilling.mockBanner":
    "Mollie keys are not configured — using test mode. Use the buttons below to simulate the payment outcome.",
  "shopBilling.openingCheckout": "Opening checkout…",
  "shopBilling.confirmingPayment": "Confirming payment…",
  "shopBilling.cycle.monthly": "Monthly",
  "shopBilling.cycle.yearly": "Yearly",

  // ── System separation labels (subscription vs booking payments) ──
  "system.subscription.label": "Subscription billing",
  "system.subscription.tagline": "Your FlowyBookings software plan",
  "system.booking.label": "Booking payments",
  "system.booking.tagline": "Payments your customers make to you",

  // ── Shop /payments page (booking payments only) ──
  "shopPayments.title": "Booking payments",
  "shopPayments.description":
    "Payments your customers make to your shop. Settled directly into your own Mollie account.",
  "shopPayments.notSubscription": "Looking for your software plan? Open Subscription billing →",
  "shopPayments.notSubscriptionTitle": "This isn't your software subscription.",
  "shopPayments.notSubscriptionBody":
    "This page handles customer payments via Mollie. Your FlowyBookings plan is managed separately.",
  "shopPayments.openBilling": "Manage subscription",
  "shopPayments.providerEmpty": "Connect Mollie below to start accepting customer payments.",
  "shopPayments.providerEmptyTitle": "No payment provider connected yet",
  "shopPayments.export": "Export",

  // ── Mollie Connect incoming booking payments ──
  "mollieConnect.payments.title": "Mollie Connect payments",
  "mollieConnect.payments.description": "Deposits collected directly into your own Mollie account.",
  "mollieConnect.payments.empty": "No Mollie Connect payments received yet.",
  "mollieConnect.payments.customer": "Customer",
  "mollieConnect.payments.amount": "Amount",
  "mollieConnect.payments.fee": "Platform fee",
  "mollieConnect.payments.status": "Status",
  "mollieConnect.payments.molliePaymentId": "Mollie ID",
  "mollieConnect.payments.actions": "Actions",
  "mollieConnect.payments.refund": "Refund",
  "mollieConnect.payments.refunding": "Working…",
  "mollieConnect.payments.refundConfirmTitle": "Refund the full amount?",
  "mollieConnect.payments.refundConfirmDesc":
    "This refunds {amount} to the customer via Mollie. This action cannot be undone.",
  "mollieConnect.payments.refundConfirm": "Yes, refund",
  "mollieConnect.payments.refundCancel": "Cancel",
  "mollieConnect.payments.refundSuccess": "Refund initiated",
  "mollieConnect.payments.refundError": "Refund failed",

  // ── Admin: Booking Payment Providers (separate page) ──
  "adminProvidersPage.title": "Booking payment providers",
  "adminProvidersPage.description":
    "Mollie Connect status, onboarding state, and platform fee config for every shop. Subscription billing lives separately under Platform billing.",
  "adminProvidersPage.connected": "Connected",
  "adminProvidersPage.pending": "Pending",
  "adminProvidersPage.notConnected": "Not connected",
  "adminProvidersPage.errors": "Errors",
  "adminProvidersPage.searchPlaceholder": "Search shop or owner email…",
  "adminProvidersPage.shopsTitle": "Shops & providers",
  "adminProvidersPage.shopsDesc":
    "One row per shop. Subscription state is NOT shown here — open Platform billing for that.",
  "adminProvidersPage.empty": "No shops match this filter.",
  "adminProvidersPage.onboarding": "Onboarding",
  "adminProvidersPage.connectedAt": "connected",
  "adminProvidersPage.scopeHint":
    "Booking payments only. For subscription plans go to Platform billing.",

  // ── Admin nav additions ──
  "adminNav.providers": "Booking providers",

  // ── Admin payments: kind separation ──
  "adminPayments.kind.all": "All payments",
  "adminPayments.kind.subscription": "Subscription only",
  "adminPayments.kind.booking": "Booking only",
  "adminPayments.kindLabel": "Type",

  // ── Inbox: notification kind label (uses metadata.kind) ──
  "inbox.kind.subscription": "Plan",
  "inbox.kind.provider": "Mollie",

  // ── Support page ──
  "support.title": "Support",
  "support.subtitle": "Help, information, and policies",
  "support.contact": "Contact us",
  "support.legal": "Legal",
  "support.legal.privacy": "Privacy Policy",
  "support.legal.terms": "Terms & Conditions",
  "support.legal.refunds": "Refund Policy",
  "support.company": "FlowyBookings · KvK: 69444552",

  // ── Support FAQ ──
  "support.faq.payments.q": "How do I receive payments?",
  "support.faq.payments.a":
    "Payments are processed through Mollie and paid directly to your connected account.",
  "support.faq.connect.q": "How do I connect Mollie?",
  "support.faq.connect.a": "Go to your dashboard → Payments → Connect Mollie and follow the steps.",
  "support.faq.payouts.q": "When do I get paid?",
  "support.faq.payouts.a": "Payouts are handled by Mollie and depend on your account settings.",
  "support.faq.cancel.q": "Can customers cancel bookings?",
  "support.faq.cancel.a": "Yes, depending on your cancellation policy set by the shop.",
  "support.faq.refunds.q": "How do refunds work?",
  "support.faq.refunds.a":
    "Refunds are handled by the shop. FlowyBookings does not process refunds directly.",

  // ── Legal: shared ──
  "legal.contact": "Contact",
  "legal.company": "Company",
  "legal.companyLine": "FlowyBookings",
  "legal.kvk": "KvK: 69444552",
  "legal.back": "Back",
  "legal.lastUpdated": "Last updated",

  // ── Legal: Privacy ──
  "legal.privacy.title": "Privacy Policy",
  "legal.privacy.metaDesc": "How FlowyBookings collects, uses, and protects your data.",
  "legal.privacy.intro":
    "FlowyBookings respects your privacy. We collect only the data necessary to provide our booking and payment services.",
  "legal.privacy.collectHeading": "Data we collect",
  "legal.privacy.collect1": "Account information (name, email)",
  "legal.privacy.collect2": "Booking data",
  "legal.privacy.collect3": "Payment-related data via Mollie (we do NOT store payment details)",
  "legal.privacy.useHeading": "We use this data to",
  "legal.privacy.use1": "Provide and improve our platform",
  "legal.privacy.use2": "Process bookings and payments",
  "legal.privacy.use3": "Communicate with users",
  "legal.privacy.shareHeading": "Sharing",
  "legal.privacy.share":
    "We do not sell or share your data with third parties, except where necessary for payment processing (Mollie).",

  // ── Legal: Terms ──
  "legal.terms.title": "Terms & Conditions",
  "legal.terms.metaDesc": "Terms and conditions for using FlowyBookings.",
  "legal.terms.intro": "By using FlowyBookings, you agree to the following:",
  "legal.terms.point1": "You are responsible for the information you provide",
  "legal.terms.point2": "Shops are responsible for their services and bookings",
  "legal.terms.point3": "FlowyBookings is not liable for disputes between customers and shops",
  "legal.terms.point4": "Payments are processed via Mollie",
  "legal.terms.suspend": "We reserve the right to suspend accounts in case of misuse.",

  // ── Legal: Refunds ──
  "legal.refunds.title": "Refund Policy",
  "legal.refunds.metaDesc": "How refunds are handled on FlowyBookings.",
  "legal.refunds.line1": "Refunds are handled by the individual shop unless stated otherwise.",
  "legal.refunds.line2":
    "FlowyBookings does not process refunds directly, as payments are handled via Mollie and the shop.",
  "legal.refunds.subHeading": "For subscription plans",

  // ── Admin: Revenue ──
  "adminNav.revenue": "Revenue",
  "adminNav.customers": "Customers",
  "adminRevenue.title": "Revenue",
  "adminRevenue.description":
    "What your platform earned this month — subscriptions + transaction fees per shop.",
  "adminRevenue.exportCsv": "Export CSV",
  "adminRevenue.totalThisMonth": "Total this month",
  "adminRevenue.subscriptionRevenue": "Subscriptions this month",
  "adminRevenue.feeRevenue": "Platform fees this month",
  "adminRevenue.mrr": "MRR",
  "adminRevenue.mrrHint": "Monthly recurring revenue",
  "adminRevenue.lifetime": "Lifetime total",
  "adminRevenue.avgFee": "Avg. fee/txn: {amount}",
  "adminRevenue.last12Months": "Last 12 months",
  "adminRevenue.stackedByType": "Subscriptions + fees stacked",
  "adminRevenue.byPlan": "Distribution by plan",
  "adminRevenue.noRevenueYet": "No revenue to show yet",
  "adminRevenue.mrrTrend": "MRR trend",
  "adminRevenue.subscriptionsOnly": "Subscription payments only",
  "adminRevenue.perShop": "Per shop",
  "adminRevenue.sortedByLifetime": "Sorted by lifetime revenue",
  "adminRevenue.shop": "Shop",
  "adminRevenue.plan": "Plan",
  "adminRevenue.subMonth": "Sub this month",
  "adminRevenue.feeMonth": "Fees this month",
  "adminRevenue.subTotal": "Sub total",
  "adminRevenue.feeTotal": "Fees total",
  "adminRevenue.txns": "Transactions",
  "adminRevenue.noShops": "No shops yet",

  // ── Admin: Customers ──
  "adminCustomers.title": "All customers",
  "adminCustomers.description": "Platform-wide list of every end customer across all shops.",
  "adminCustomers.exportCsv": "Export CSV",
  "adminCustomers.searchPlaceholder": "Search by name, email, phone, shop…",
  "adminCustomers.filterByShop": "Filter by shop",
  "adminCustomers.allShops": "All shops",
  "adminCustomers.totalCount": "{count} customers",
  "adminCustomers.name": "Name",
  "adminCustomers.contact": "Contact",
  "adminCustomers.shop": "Shop",
  "adminCustomers.visits": "Visits",
  "adminCustomers.spent": "Spent",
  "adminCustomers.lastVisit": "Last visit",
  "adminCustomers.signedUp": "Signed up",
  "adminCustomers.noResults": "No customers found",
  "adminNav.sms": "SMS balance",
  "adminSms.title": "SMS balance per shop",
  "adminSms.description": "Overview of credits, usage and missed reminders.",
  "adminSms.shopsAtZero": "Shops at 0 credits",
  "adminSms.skipped30d": "Skipped SMS (30d)",
  "adminSms.resumedShops": "Shops ever resumed",
  "adminSms.searchPlaceholder": "Search by shop name or slug…",
  "adminSms.sortLongestZero": "Longest at zero",
  "adminSms.sortBalanceAsc": "Lowest balance",
  "adminSms.sortSkippedDesc": "Most skipped",
  "adminSms.sortName": "Name",
  "adminSms.shop": "Shop",
  "adminSms.balance": "Balance",
  "adminSms.purchased": "Purchased",
  "adminSms.used": "Used",
  "adminSms.skipped": "Skipped 30d",
  "adminSms.zeroSince": "At zero since",
  "adminSms.lastResumed": "Last resumed",
  "adminSms.noShops": "No shops found",

  // ── Feature gating (FeatureLock + plan labels) ──
  "plan.trial": "Trial",
  "plan.starter": "Starter",
  "plan.pro": "Pro",
  "plan.premium": "Premium",
  "plan.fallback": "a higher plan",
  "plan.priceMonthly": "€{price}/month",
  "featureLock.usageWarning": "You've used {used}/{limit} {feature} this month.",
  "featureLock.usageUpgrade": "Upgrade to {plan} for a higher limit.",
  "featureLock.upgradeBtn": "Upgrade",
  "featureLock.upgradeTo": "Upgrade to {plan}",
  "featureLock.titleLimit": "Your {feature} limit has been reached",
  "featureLock.titlePlan": "{feature} — available from {plan}",
  "featureLock.titlePlanWithPrice": "{feature} — available from {plan} ({price})",
  "featureLock.descLimit":
    "You've used {used}/{limit} {feature} this month. Upgrade for more or wait until next month.",
  "featureLock.descPlan":
    "This feature isn't in your current plan ({plan}). Upgrade to activate it.",
  "feature.bookings": "bookings",
  "feature.staff": "staff",
  "feature.smsReminders": "SMS reminders",
  "feature.whatsappReminders": "WhatsApp reminders",
  "feature.customBranding": "Custom branding",
  "feature.marketingEmails": "Marketing emails",
  "feature.advancedAnalytics": "Advanced analytics",

  // ── Staff page (extra) ──
  "staff.limitTooltip": "Your plan supports up to {limit} staff member. Upgrade to add more.",
  "staff.limitTooltipPlural":
    "Your plan supports up to {limit} staff members. Upgrade to add more.",

  // ── Calendar page (extra) ──
  "calendar.amount": "Amount",
  "calendar.paymentSectionTitle": "Payment",
  "calendar.bookingSummary": "Summary",
  "calendar.totalPrice": "Total price",
  "calendar.depositAmount": "Deposit",
  "calendar.dueAtAppointment": "Due at appointment",
  "calendar.noDepositRequired": "No deposit required — full amount due at the appointment.",
  "calendar.noOnlinePaymentManual": "Manual booking — no online payment; settle the deposit in person.",
  "calendar.manualBookingBadge": "Manual",
  "calendar.onlineBookingBadge": "Online",
  "calendar.today": "Today",
  "calendar.appointment": "appointment",
  "calendar.appointments": "appointments",
  "calendar.trialExpiredBookingTitle":
    "Your trial has expired — pick a plan to create new appointments.",
  "calendar.bookingLimitReached":
    "You've reached the maximum number of bookings ({used}/{limit}). Pick a plan to continue.",
  "calendar.workingToday": "Working today",
  "calendar.dayOff": "Off today",
  "calendar.firstAvailableSlot": "Next available slot",
  "calendar.firstAvailableSlotTooltip": "Find the next available slot for this staff member",
  "calendar.firstAvailablePickStaff": "Pick a staff member first",
  "calendar.firstAvailablePickDuration": "Pick a duration",
  "calendar.firstAvailableNone": "No available slot found in the next 7 days.",
  "calendar.firstAvailableFound": "Next available slot: {when}",
  "calendar.rescheduleTitle": "Reschedule booking",
  "calendar.rescheduleDesc": "Pick a new start time. Duration and staff stay the same.",
  "calendar.rescheduleNewStart": "New start",
  "calendar.rescheduleConfirm": "Reschedule",
  "calendar.rescheduling": "Rescheduling…",
  "calendar.rescheduleSamePicked": "Pick a different time to reschedule.",
  "calendar.pullToRefresh": "Pull to refresh",
  "calendar.releaseToRefresh": "Release to refresh",
  "calendar.refreshing": "Refreshing…",
  "calendar.remainingBalance": "Remaining balance",
  "calendar.unassigned": "Unassigned",
  "calendar.appointmentDetails": "Appointment details",
  "calendar.allergyNote": "Allergy / note",
  "calendar.viewDay": "Day",
  "calendar.viewWeek": "Week",
  "calendar.viewWeekDisabledMobile": "Week view is only available on tablet/desktop",
  "calendar.viewGrid": "Schedule",
  "calendar.viewGridDisabledPickDay": "Pick a day to show the schedule",
  "calendar.viewList": "List",
  "calendar.prevWeek": "Previous week",
  "calendar.nextWeek": "Next week",
  "calendar.jumpToThisWeek": "Go to this week",
  "calendar.thisWeek": "This week",
  "calendar.staffOffToday": "{staff} isn't working today",
  "calendar.unavailableStaffOffToday": "Unavailable — {staff} isn't working today",
  "calendar.outsideHours": "Outside working hours",
  "calendar.outsideHoursRangeCompact": "Outside working hours {range}",
  "calendar.outsideHoursStaffRange": "Outside working hours — {staff} works {range}",
  "calendar.break": "Break",
  "calendar.breakRangeCompact": "Break {range}",
  "calendar.unavailableStaffTime": "Unavailable — {staff} {time}",
  "calendar.newBookingStaffTime": "New booking {staff} {time}",
  "calendar.dragOrKeysToMove": "Drag or use arrow keys to move",
  "calendar.arrowKeysHint": "Arrow keys: ±15 min or staff column",
  "calendar.openDayView": "Open day view",
  "calendar.shopClosed": "Shop closed",
  "calendar.arrowKeysMove": "Arrow keys to move",
  "calendar.arrowKeysMoveDayHint": "Arrows: ±15 min or ±1 day",
  "calendar.closedBadge": "Closed",
  "calendar.timeColumn": "Time",
  "calendar.staffBreakAtTime": "{staff} has a break at that time",
  "calendar.outsideStaffHours": "Outside {staff}'s working hours",
  "calendar.pickAvailableTimeHint": "Choose a time within working hours, or adjust the staff member's schedule.",
  "calendar.cancelActionTitle": "Cancel this booking?",
  "calendar.cancelActionDesc": "The customer will be notified by email. This can't be undone.",
  "calendar.cancelBooking": "Cancel booking",
  "calendar.cancelReasonLabel": "Reason for cancellation",
  "calendar.cancelReasonPlaceholder": "e.g. customer requested, staff unavailable…",
  "calendar.capturedAmount": "Amount captured",
  "calendar.completedActionTitle": "Mark as completed?",
  "calendar.completedActionDesc": "This won't notify the customer.",
  "calendar.confirmActionTitle": "Confirm this booking?",
  "calendar.confirmActionDesc": "The customer will receive a confirmation email.",
  "calendar.actionConfirmBooking": "Confirm booking",
  "calendar.actionMarkCompleted": "Mark completed",
  "calendar.actionMarkConfirmed": "Mark as confirmed",
  "calendar.actionMarkNoShow": "Mark as no-show",
  "calendar.moreActions": "More actions",
  "calendar.noActionsAvailable": "No actions available right now",
  "calendar.durationMinutesOnly": "{{min}} min",
  "calendar.durationHoursMinutes": "{{h}}h {{m}}m",
  "calendar.confirmMoveAction": "Confirm",
  "calendar.confirmMoveTitle": "Move to {{time}}?",
  "calendar.confirmMoveDesc": "The customer will be notified by email.",
  "calendar.confirmResizeTitle": "Change end time to {{time}}?",
  "calendar.confirmResizeDesc": "No email will be sent — the start time isn't changing.",
  "calendar.genericError": "Something went wrong. Please try again.",
  "calendar.noShowActionTitle": "Mark as no-show?",
  "calendar.noShowActionDesc": "This won't notify the customer.",
  "calendar.refundedBadge": "Refunded",
  "calendar.undoActionTitle": "Mark as confirmed again?",
  "calendar.undoActionDesc": "This won't notify the customer.",
  "calendar.guardReason.booking_already_started": "This appointment has already started.",
  "calendar.guardReason.booking_not_started_yet": "This appointment hasn't started yet.",
  "calendar.guardReason.booking_not_ended_yet": "This appointment hasn't ended yet.",
  "calendar.guardReason.payment_in_progress": "Waiting for the online payment to complete.",
  "calendar.guardReason.invalid_current_status": "This action isn't available for the current status.",
  "calendar.guardReason.missing_reason": "A reason is required.",
  "calendar.guardReason.unauthenticated": "You need to sign in again.",
  "calendar.guardReason.forbidden": "You don't have permission to do this.",
  "calendar.guardReason.booking_not_found": "This booking could not be found.",
  "calendar.guardReason.status_changed": "This booking was just changed elsewhere — please try again.",
  "calendar.guardReason.invalid_action": "That action isn't recognized.",
  "billing.trialExpiredTitle": "Your free trial has expired",
  "billing.trialExpiredSub":
    "You can no longer create new appointments. Existing data stays visible. Pick a plan to continue.",
  "billing.trialDaysLeft": "{{days}} days left in your free trial",
  "billing.trialDaysLeftSub":
    "Pick a plan before your trial expires to keep accepting appointments.",
  "billing.trialActiveTitle": "Your free trial is active (14 days)",
  "billing.trialActiveSub": "No charges until {{date}} — every feature stays unlocked.",
  "billing.trialEndingTitle": "Your trial ends in {{days}} days",
  "billing.trialEndingSub": "Ends on {{date}}. Pick your plan to continue.",
  "billing.nextPayment": "Next charge: {{amount}} on {{date}}",
  "billing.pendingPayment": "Payment pending: {{amount}} — Mollie collects on {{date}}",
  "billing.securePayments": "Payments are securely processed by Mollie",
  "shopBilling.cycle.short.monthly": "per month",
  "shopBilling.cycle.short.yearly": "per year",
  "billing.paymentFailedTitle": "Payment failed — update your payment method",
  "billing.paymentFailedSub":
    "We couldn't process your last payment. You have {{days}} days before new bookings are blocked.",
  "billing.paymentFailedBlockedTitle": "Bookings blocked — payment failed for over 7 days",
  "billing.paymentFailedBlockedSub":
    "Update your payment method to start accepting appointments again. Existing data stays visible.",
  "billing.lapsedBlockedTitle": "Subscription lapsed — bookings blocked",
  "billing.lapsedBlockedSub":
    "Your plan ended and wasn't renewed. Pick a plan to start accepting appointments again. Existing data stays visible.",
  "billing.planActiveUntilTitle": "Your {{plan}} plan runs until {{date}}",
  "billing.planActiveUntilSub":
    "You keep every feature until that date. Reactivate any time — nothing is lost.",
  "calendar.upcomingSuffix": "upcoming",
  "calendar.todaySuffix": "today",
  "calendar.zeroToday": "No appointments today",
  "calendar.zeroOnDay": "No appointments on {{day}}",
  "calendar.onDayPrefix": "on",
  "calendar.emptyTitle": "No appointments to show",
  "calendar.emptyNoStaffSelected": "Pick a staff member to view their appointments.",
  "calendar.emptyNoStaffActive": "Add an active staff member to start your schedule.",
  "calendar.filterLabel": "Filter",
  "calendar.unassignedShort": "Unassigned",
  "billing.choosePlan": "Pick a plan",
  "billing.viewPlans": "View plans",
  "billing.updatePayment": "Update payment method",
  "billing.reactivate": "Reactivate",
  "billing.keepCurrentPlan": "Keep current plan",
  "billing.downgradeCancelSuccess": "Scheduled downgrade cancelled — staying on your current plan.",
  "billing.cancel": "Cancel subscription",
  "billing.cancelConfirm":
    "Are you sure you want to cancel? Your plan stays active until the end of the current period.",
  "billing.cancelSuccess": "Subscription cancelled. Active until end of period.",
  "billing.statusCancelled": "Cancelled",
  "billing.statusPaymentFailed": "Payment failed",
  "billing.statusPending": "Activating",
  "billing.activating": "activating…",
  "billing.activatingShort": "ACTIVATING…",
  "impersonate.readOnlyTooltip": "Read-only while impersonating",
  "mobileSheet.view": "View",
  "mobileSheet.edit": "Edit",
  "mobileSheet.duplicate": "Duplicate",
  "mobileSheet.delete": "Delete",
  "mobileSheet.actions": "Actions",
  "common.filter": "Filter",
  "common.all": "All",
  "fees.title": "Transaction fees",
  "fees.subtitle": "Fully transparent — you always see exactly what you pay for.",
  "fees.statPlan": "Current plan",
  "fees.statPerBooking": "Per paid booking",
  "fees.statPaidThisMonth": "Paid bookings this month",
  "fees.statTotalThisMonth": "Total this month",
  "fees.explainer.starter":
    "You pay €0.50 per paid booking on Starter. Only successful payments count. Cancelled or failed payments are never charged.",
  "fees.explainer.pro":
    "You pay €0.30 per paid booking on Pro. Only successful payments count. Cancelled or failed payments are never charged.",
  "fees.explainer.premium":
    "No transaction fees per booking. On Premium you pay 0% on your booking revenue.",
  "fees.explainer.trial":
    "No transaction fees during your free trial. After upgrading you'll see exactly what you pay per booking here.",
  "fees.trust.noHidden": "No hidden fees",
  "fees.trust.onlyPaid": "Only paid bookings count",
  "fees.trust.alwaysClear": "Always clear what you pay for",
  "fees.recentTitle": "Last 10 bookings with fee",
  "fees.recentEmpty": "No paid bookings yet — as soon as a customer pays you'll see it here.",
  "fees.itemBooking": "booking",
  "fees.itemFee": "fee",
};
